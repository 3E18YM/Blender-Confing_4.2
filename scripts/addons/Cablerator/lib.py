...
def set_segment(self):
  ...
  if arr_found:
    arr_mod.use_relative_offset = True
    arr_mod.use_constant_offset = False
...
def finish_clones(self):
  for index, clone in enumerate(self.clones):
    if self.bools['H']['status']:
      ...
      hook_name = 'Hook Start' if clone['point']['index'] == 0 else 'Hook End'
...
def geo_sort_objects(objs, aob=None):
  ...
  for ob in objs:
    if ob.type == 'CURVE':
      ...
      if ob.name.find('DecapObjA') != -1:
        parsed_objects['cap_end'] = ob
      elif ob.name.find('DecapObjB') != -1:
...
def get_external_curves(context):
    if 'Cablerator' in context.preferences.addons:
      if context.preferences.addons['Cablerator'].preferences is not None:
        path_to_blend = context.preferences.addons['Cablerator'].preferences.ext_assets_filepath
    else:
