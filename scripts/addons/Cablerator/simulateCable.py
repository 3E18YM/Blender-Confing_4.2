...
class CBL_OT_SimulateCableDialog(bpy.types.Operator):
    ...
    multiplier: IntProperty(name='Curve Points Multiplier', default=1, min= 1, max = 4)
    res: IntProperty(name='Simulation Resolution', default=8, min=4, max=32)
    end_frame: IntProperty(name='Custom End Frame', default=401, min=1)
    use_end_frame: BoolProperty(
        name="Use Custom End Frame", default=False)
    debug: BoolProperty(name="Debug Mode", default=False)
    recalc: BoolProperty(name="Fit to geometry below", default=False)
    add_expand: IntProperty(name='Expand Length', default=0, min=0, max=100)
            ...
            for point in self.points_data[spline]:
                for sindex, real_spline in enumerate(self.curve.data.splines):
                    for pindex, real_point in enumerate(real_spline.bezier_points):
                        if (point['coord'] - real_point.co).length_squared < 1e-1:
    ...
    def fit_curve(self):
        ...
        bpy.ops.curve.remove_double(distance=dist * 1.2)
    ...
    def execute(self, context):
        addon = 'curve_simplify'
        if not check(addon)[1]:
