...
def check_for_updates():
    addon_date = tuple(bpy.context.preferences.addons['Cablerator'].preferences.update_date)
    for mod in addon_utils.modules():
        if mod.bl_info.get('name') == 'Cablerator':
            addon_version = mod.bl_info.get('version', (-1, -1, -1))
    cur_date = tuple(datetime.today().timetuple()[:3])
    if cur_date > addon_date:
        bpy.context.preferences.addons['Cablerator'].preferences.should_update = check_for_update(addon_version)
        bpy.context.preferences.addons['Cablerator'].preferences.update_date = cur_date
    else:
        pass
...
def check_file_name(self, context):
    f = context.preferences.addons['Cablerator'].preferences.ext_assets_filepath
    if f == '':
        context.preferences.addons['Cablerator'].preferences.ext_assets_filepath = get_path_to_blend()
...
class createCablePrefs(bpy.types.AddonPreferences):
    ...
    def draw(self, context):
        ...
        if self.tabs == "PREFS":
            ...
            column_main.prop(self, 'show_offset')
            column_main.prop(self, 'show_length')
            column_main.prop(self, 'show_grab_profile')
            column_main.prop(self, 'ext_assets_filepath')
...
class VIEW3D_MT_cablerator_helper(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        layout.separator()
        layout.operator("cbl.convert_curve_to_gn", text="Convert Curve to Geo Nodes Object")
class VIEW3D_MT_cablerator(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        layout = self.layout
        layout.operator_context = 'INVOKE_REGION_WIN'
        if bpy.context.preferences.addons['Cablerator'].preferences.should_update != '':
            layout.label(text=bpy.context.preferences.addons['Cablerator'].preferences.should_update, icon='ERROR')
            layout.separator()
        ...
        myop.use_method=-1
        myop.use_length=-1
        layout.operator("object.cableratordraw", text="Draw Cable", icon_value=icons_dict["draw_cable_icon"].icon_id)
        layout.operator("cbl.massive_cables", text="Create a Mass from Selected Faces", icon_value=icons_dict["create_massive_icon"].icon_id)
        layout.operator("object.cablerator_create_cable_from_edge", text="Create from Edges", icon_value=icons_dict["create_from_edges_icon"].icon_id)
        layout.operator("object.cablerator_create_cable_from_selected", text="Create from Selected Objects", icon_value=icons_dict["create_from_objs_icon"].icon_id)
        layout.separator()
        layout.operator("object.cablerator_edit_cable", text="Edit Cable", icon_value=icons_dict["edit_cable_icon"].icon_id)
        layout.operator("object.cableratorconnect", text="Merge End Points", icon_value=icons_dict["merge_points_icon"].icon_id)
        layout.separator()
        layout.operator("object.cablerator_connector", text="Add Connectors", icon_value=icons_dict["add_connectors_icon"].icon_id)
        layout.operator("object.cablerator_segment", text="Add or Edit Segment", icon_value=icons_dict["add_segment_icon"].icon_id).duplicate = False
        layout.operator("object.cablerator_geocable", text="Convert Curve to Mesh Cable", icon_value=icons_dict["add_mesh_icon"].icon_id)
        layout.separator()
        layout.operator("object.cableratorsplit", text="Split Cable by Profiles", icon_value=icons_dict["split_cable_icon"].icon_id)
        layout.operator("object.cableratorsplitrecable", text="Rebuild Split Cable", icon_value=icons_dict["split_rebuild_icon"].icon_id)
        layout.separator()
        layout.operator("cbl.simulate_cable_dialog", text="Simulate Cable...", icon_value=icons_dict["simulate_icon"].icon_id)
        layout.separator()
        layout.operator("cbl.insulate", text="Insulate", icon_value=icons_dict["insulate_icon"].icon_id)
        layout.operator("cbl.rope", text="Rope", icon_value=icons_dict["rope_icon"].icon_id)
        layout.separator()
        layout.menu(VIEW3D_MT_cablerator_hooks.bl_idname, icon="HOOK")
        layout.menu(VIEW3D_MT_cablerator_helper.bl_idname, icon_value=icons_dict["misc_icon"].icon_id)
