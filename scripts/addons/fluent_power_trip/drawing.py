...
class FLUENT_Drawing():
    ...
    def __init__(self):
        ...
        self.rectangle_info = [['Draw from center', 'SHIFT'], ['Draw a square', 'CTRL'], ['Validate', 'LEFT CLICK'], ['Validate path', 'Space'], ['Cancel', 'ESC']]
        self.prism_info = [['Force triangle', 'Hold CTRL'], ['Force hexagon', 'Hold ALT'], ['Validate', 'LEFT CLICK'], ['Validate as shape', 'SHIFT + LEFT CLICK'], ['Cancel', 'ESC']]
        self.shape_info = [['Snap direction', 'HOLD CTRL'], ['Remove', 'BACKSPACE'], ['Validate shape', 'SHIFT + LEFT CLICK'], ['Validate path', 'SPACE'], ['Cancel', 'ESC']]
        self.revolver_info = [['Remove', 'BACKSPACE'], ['Validate path', 'SPACE'], ['Cancel', 'ESC']]
    ...
    def grid_init(self, obj_original, events):
        if obj_original and not self.align_to_view:
            ...
            # suppression du dernier bevel pour avoir des arrêtes nettes
            remove_me = {'bevel':None, 'weighted_normal':None}
            for m in reversed(copy.modifiers):
                if not remove_me['bevel'] and m.type == 'BEVEL' and m.name == fluent_modifiers_name['outer_bevel']:
                    remove_me['bevel'] = m
                elif not remove_me['weighted_normal'] and m.type == 'WEIGHTED_NORMAL':
                    remove_me['weighted_normal'] = m
                if remove_me['bevel'] and remove_me['weighted_normal']:
                    break
            if remove_me['bevel']:
                copy.modifiers.remove(remove_me['bevel'])
            if remove_me['weighted_normal']:
                copy.modifiers.remove(remove_me['weighted_normal'])
            ...
            if not len(self.sommets_dans_le_plan):
                make_oops(['No vertices aligned on the face.', 'The face isn\'t flat.', 'Impossible to display the gid on the face'], 'Error', 'ERROR')
                bpy.data.objects.remove(copy, do_unlink=True)
                return False
    ...
    def set_the_draw_type(self, a):
        ...
        if self.the_draw['type'] == 'RECTANGLE':
            self.side_infos.add_line('RECTANGLE', '', reset = True)
            self.side_infos.add_line('Draw a rectangle', 'Left click', reset = False)
            self.side_infos.add_line('Draw from outside', 'Shift + Left click on a face', reset = False)
        elif self.the_draw['type'] == 'PRISM':
            self.side_infos.add_line('CIRCLE', '', reset = True)
            self.side_infos.add_line('Draw a circle', 'Left click', reset = False)
            self.side_infos.add_line('Draw from outside', 'Shift + Left click on a face', reset = False)
        elif self.the_draw['type'] == 'SHAPE':
            self.side_infos.add_line('SHAPE', '', reset = True)
            self.side_infos.add_line('Draw a shape', 'Left click', reset = False)
            self.side_infos.add_line('Draw from outside', 'Shift + Left click on a face', reset = False)
        elif self.the_draw['type'] == 'REVOLVER':
            self.side_infos.add_line('Solid of revolution', '', reset = True)
            self.side_infos.add_line('Display the grid', 'Right click', reset = False)
            self.side_infos.add_line('Set the pivot point', 'Right click again', reset = False)
    ...
    def process(self, events):
        ...
        if self.statut == 'GRILLE_OK':
            ...
            if events['type'] == 'LEFTMOUSE' and events['value'] == 'RELEASE' and not events['shift_work']:
                cast = obj_ray_cast(self.drawing_plane, mouse_x, mouse_y)
                if cast['success']:
            ...
            if events['type'] == 'LEFTMOUSE' and events['value'] == 'RELEASE':
                cast = obj_ray_cast(self.drawing_plane, mouse_x, mouse_y)
                if cast['success']:
                    self.the_draw['coords_premier_clique'] = cast['hit']
    ...
    def draw(self, events):
        ...
        if self.display_grid:
            ...
            if self.display_dots:
                try:
                    ...
                    draw_line(coords=global_coords, thickness = 2, color = (0, .75, 1, 1), is_2d = False)
                except:pass
        ...
        if self.statut == 'EN_COURS':
            ...
            draw_edges(self.the_draw['obj'], 2, (1, 1, 1, 1))
