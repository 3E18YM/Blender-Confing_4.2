...
class FLUENT_OT_ClothPanel(Operator):
    ...
    def modal(self, context, event):
        ...
        if not event.type in ['RET', 'NUMPAD_ENTER', 'ESC'] and bpy.context.active_object and bpy.context.active_object.mode == 'EDIT':
            return {'PASS_THROUGH'}
    ...
    def invoke(self, context, event):
        ...
        if get_addon_preferences().cloth_remesh_tool == 'QUADREMESHER':
            try:
                ...
                if str(e) == "'Scene' object has no attribute 'qremesher'":
                    make_oops(['You selected QuadRemesher in the preferences.', 'QuadRemesher seems to be desactivated.', 'Thanks to active it or change the remesher tool.'], 'How to use ?', 'ERROR')
                    return {'FINISHED'}
        elif get_addon_preferences().cloth_remesh_tool == 'INSTANT_MESH':
            ...
            if OS == 'Windows' and not 'Instant Meshes.exe' in exe:
                make_oops(['Instant Mesh path is wrong.',
                           'Check the preferences.',
                           'For Windows : Instant mesh.exe is expected'], 'How to use ?', 'ERROR')
                return {'FINISHED'}
            elif OS == 'Darwin' and not '.app/Contents/MacOS/Instant Meshes' in exe:
                make_oops(['Instant Mesh path is wrong.',
                           'Check the preferences.',
                           'For Mac : .app/Contents/MacOS/Instant Meshes is expected'], 'How to use ?', 'ERROR')
                return {'FINISHED'}
            elif OS == 'Linux' and not exe:
                make_oops(['Instant Mesh path is empty.', 'Check the preferences.'], 'How to use ?', 'ERROR')
                return {'FINISHED'}
        ...
        side_infos.add_line('Cancel', 'Esc')
...
class Remesh_Mangement():
    ...
    def remesh_trigger(self):
        ...
        if self.remesher == 'INSTANT_MESH' and self.instant_mesh_data.get('exported') and self.instant_mesh_data['exported']:
            try:
                bpy.ops.import_scene.obj(filepath=self.instant_mesh_data['output'],
