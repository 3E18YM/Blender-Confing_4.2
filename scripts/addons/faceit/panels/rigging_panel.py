    ...
    def draw(self, context):
        ...
        if adaption_state == 0:
            ...
            if scene.faceit_workspace.active_tab == 'CONTROL':
                ...
                row = col.row(align=True)
                grp_prop = bpy.context.scene.faceit_vertex_groups['faceit_main']
                picker_running = grp_prop.picker_running
                # picker_running = is_picker_running()
                row.operator('faceit.vertex_group_picker', text='Set Main Group',
    ...
    def draw(self, context):
        ...
        if scene.faceit_eye_pivot_placement == 'AUTO':
            ...
            if scene.faceit_eye_geometry_type == 'SPHERE':
                ...
                grp_prop = bpy.context.scene.faceit_vertex_groups['faceit_left_eyeball']
                op = row.operator('faceit.vertex_group_picker', text='',
                                  icon='EYEDROPPER', depress=grp_prop.picker_running)
                op.vertex_group_name = 'left_eyeball'
                op.single_surface = grp_prop.single_surface
                op.is_pivot_group = True
                op.additive_group = grp_prop.additive_group
                ...
                grp_prop = bpy.context.scene.faceit_vertex_groups['faceit_right_eyeball']
                op = row.operator('faceit.vertex_group_picker', text='',
                                  icon='EYEDROPPER', depress=grp_prop.picker_running)
