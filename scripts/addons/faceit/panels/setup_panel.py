...
class FACEIT_PT_SetupRegister(FACEIT_PT_BaseSetup, bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if not scene.faceit_face_objects:
            ...
            if scene.faceit_use_rigify_armature:  # or scene.faceit_eye_pivot_options == 'COPY_PIVOT'
                row.enabled = False
...
def draw_assign_group_options(row, grp_name, grp_name_ui, can_pick=True, is_pivot_group=False):
    ...
    single_surface = grp_prop.single_surface
    if 'main' in grp_name:
...
class FACEIT_PT_SetupVertexGroups(FACEIT_PT_BaseSetup, bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        draw_assign_group_options(row, 'right_eyes_other', 'Right Eye', is_pivot_group=True)

        row = grid.row(align=True)
        draw_assign_group_options(row, 'left_eyes_other', 'Left Eye', is_pivot_group=True)
