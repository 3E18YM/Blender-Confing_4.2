...
class FACEIT_OT_ProjectLandmarks(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if not found_pos:
            self.report({'WARNING'}, " Pivot points are only estimated. Please assign the eyeball vertex groups or use manual placement.")
...
def _reset_snap_settings(context):
    ...
    if bpy.app.version[0] < 4:
        ...
        # scene.tool_settings.use_snap_selectableFACE_PROJECT
        # scene.tool_settings.snap_elements_base = {'FACE'}
