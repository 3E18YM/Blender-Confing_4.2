...
from ..core.vgroup_utils import (apply_vertex_group, assign_vertex_grp, copy_vertex_groups,
                                 get_deform_bones_from_armature,
                                 invert_vertex_group_weights, store_vertex_group)
    ...
    def execute(self, context):
        ...
        if self.merge_faceit_weights:
            ...
            for obj in objects_to_process:
                ...
                # Hide all modifiers
                def_face_grp_dup = obj.vertex_groups.get('DEF-face')
                # Get the body weights to be normalized
                if def_face_grp_dup:
                    def_face_inv = invert_vertex_group_weights(obj, def_face_grp_dup)
                    obj.vertex_groups.remove(def_face_grp_dup)
                else:
                    vs = [v.index for v in obj.data.vertices]
                    assign_vertex_grp(obj, vs, 'DEF-face_invert')

                # Remove all vertex groups that are not relevant
                relevant_groups = set(body_deform_groups)
                relevant_groups = relevant_groups - set(all_faceit_deform_groups)
                # if relevant_groups
                relevant_groups.add('DEF-face_invert')
                stored_other_groups = {}
                found_grps = 0
                for grp in obj.vertex_groups:
                    if grp.name in relevant_groups:
                        grp.lock_weight = False
                        found_grps += 1
                    else:
                        # Remove irrelevant groups before normalizing.
                        stored_other_groups[grp.name] = store_vertex_group(obj, grp)
