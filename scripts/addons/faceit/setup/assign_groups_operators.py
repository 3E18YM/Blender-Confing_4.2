...
from ..properties.setup_scene_properties import FaceitVertexGroups
...
class FACEIT_OT_VertexGroupPicker(bpy.types.Operator):
    ...
    def __init__(self):
        ...
        self.mods_disabled = {}
        self.grp_prop: FaceitVertexGroups = None
        self.grp_name: str = ""
                                ...
                                for assigned_grp_name, assigned_overlap in overlapping_groups.items():
                                    ...
                                    if assigned_vgroup:
                                        ...
                                        if not any(assigned_vgroup.index in [g.group for g in v.groups]
                                            ...
                                            assigned_grp_prop = vgroup_props.get(assigned_grp_name)
                                            if assigned_grp_prop:
                                                warning = True
                                                if not grp_warning:
                                                    self.report(
                                                        {'WARNING'},
                                                    ...
                                                    grp_warning = True
                                                assigned_grp_prop.remove_object(obj.name)
    ...
    def end(self, context):
        ...
        self.grp_prop.picker_running = False
    ...
    def invoke(self, context, event):
        ...
        if context.area.type == 'VIEW_3D':
            ...
            vgroup_props = bpy.context.scene.faceit_vertex_groups
            self.grp_prop = vgroup_props.get('faceit_' + self.vertex_group_name)
            self.grp_prop.picker_running = True
        else:
...
class FACEIT_OT_DrawFaceitVertexGroup(bpy.types.Operator):
    ...
    def modal(self, context, event):
        ...
        if event.type in {'LEFTMOUSE', 'RIGHTMOUSE', 'ESC'}:
            ...
            self.bm.free()
            # self.report({'INFO'}, "Cancelled.")
            grp_prop = context.scene.faceit_vertex_groups.get(self.faceit_vertex_group_name)
            if grp_prop:
                grp_prop.is_drawn = False
            return {'CANCELLED'}
        return {'PASS_THROUGH'}
