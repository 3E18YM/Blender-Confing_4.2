...
class MocapBase:
        ...
        if head_obj:
            ...
            if self.head_obj.type == 'ARMATURE':
                if head_bone_name:
                    ...
                    self.initial_head_rotation = world_head_mat.to_quaternion()  # self.head_bone.matrix.inverted()
                    if self.head_bone:
    ...
    def _rotation_to_blender(self, value=None):
        ...
        # convert UE to Blender coordinates
...
class MocapImporterBase(FaceRegionsBaseProperties):
    ...
    def draw(self, context):
        ...
        if getattr(self, "live_link_face_frame_rate", None):
            row = col.row(align=True)
            row.prop(self, "live_link_face_frame_rate")
        self._draw_head_motion_types(col)
        if self.animate_head_location or self.animate_head_rotation:
            row = col.row(align=True)
            row.label(text="See Head Setup for Target Selection")

            # self.draw_head_targets_layout(col, scene=context.scene,
            #                               show_head_action=False)
    ...
    def execute(self, context):
        ...
        if animate_shapes:
            ...
            if not all_set_target_shapes.intersection(available_shape_keys):
        ...
        if getattr(self, 'live_link_face_frame_rate', None):
            self.record_frame_rate = int(self.live_link_face_frame_rate)
        ...
        if self.set_scene_frame_range:
            ...
            if animate_shapes and mocap_importer.sk_action:
                scene.frame_start, scene.frame_end = (int(x)
                                                      for x in futils.get_action_frame_range(
                                                          mocap_importer.sk_action))
        # ----------- Load Audio ----------------
        if self.load_audio_file:
