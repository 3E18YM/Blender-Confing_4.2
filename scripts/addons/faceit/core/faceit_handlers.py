...
from ..core.faceit_utils import get_faceit_objects_list, get_faceit_armature
...
def faceit_scene_update_handler(scene):
    ...
    if getattr(context.active_operator, "bl_idname", "") == "OBJECT_OT_delete":
    ...
    elif scene.faceit_expression_list and scene.faceit_workspace.active_tab in ('BAKE'):
...
def register_vertex_group_defaults(scene):
    ...
    for grp in FACEIT_VERTEX_GROUPS:
        ...
        if item is None:
            ...
            item.name = grp
        if any(x in grp for x in ('main', 'left_eyeball', 'right_eyeball')):
            item.single_surface = True
            item.additive_group = True
