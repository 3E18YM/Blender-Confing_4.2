...
def copy_vertex_groups(source_obj, target_obj):
    """
    Copy vertex groups from source_obj to target_obj. 
    Both objects should have the same topology and vertex count.

    Args:
    - source_obj (bpy.types.Object): Object from which vertex groups are copied.
    - target_obj (bpy.types.Object): Object to which vertex groups are copied.
    """
    # Copy vertex groups from source to target
    for vg in source_obj.vertex_groups:
        # if vg.name in target_obj.vertex_groups:
...
def store_vertex_group(obj, vertex_group):
    '''Store the vertex group data in a list of tuples (vertex_index, weight)'''
    vertex_data = []
    for v in obj.data.vertices:
        try:
            weight = vertex_group.weight(v.index)
            vertex_data.append((v.index, weight))
        except RuntimeError:
            continue  # vertex not in group, so just skip it
    return vertex_data
    # return [(v.index, vertex_group.weight(v.index)) for v in obj.data.vertices if vertex_group.weight(v.index) > 0]
