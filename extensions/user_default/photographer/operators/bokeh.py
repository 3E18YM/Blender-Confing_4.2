...
from ..constants import HDR_FORMATS
from ..functions.functions import copy_collections, show_message
                            ...
                            if type(node) is bpy.types.ShaderNodeTexImage:
                                if node.get("is_bokeh_tex", False):
                                    if self.bokeh_tex != 'empty':
                                        ...
                                        if self.bokeh_tex.lower().endswith(HDR_FORMATS):
...
class PHOTOGRAPHER_OT_Bokeh_Add(bpy.types.Operator):
    ...
    def execute(self,context):
        ...
        if mat is None:
            ...
            br.location = (400,100)
            br.name = br.label = "Brightness"

            # Saturation
            sat = nodes.new('ShaderNodeValue')
            sat.location = (400,200)
            sat.name = sat.label = "Saturation"
            ...
            # Connect them
            links.new(img.outputs[0], hsv.inputs[4])
            links.new(br.outputs[0], hsv.inputs[2])
            links.new(sat.outputs[0], hsv.inputs[1])
            links.new(hsv.outputs[0], transp.inputs[0])
            ...
            # Saturation
            d = sat.outputs[0].driver_add('default_value').driver

            var = d.variables.new()
            var.name = 'bokeh_saturation'
            target = var.targets[0]
            target.id_type = 'CAMERA'
            target.id = cam
            target.data_path = 'photographer.bokeh_saturation'

            d.expression = "bokeh_saturation"
...
class PHOTOGRAPHER_OT_OptVignetting_Add(bpy.types.Operator):
    ...
    def execute(self,context):
        ...
        d.expression = "tan(atan(lens_shift/(fl/sh)))*"+loc_z+" if (sf=='2') else tan(atan(lens_shift/(fl/sw)))*"+loc_z
...
class PHOTOGRAPHER_OT_FixDisplayType(bpy.types.Operator):
    bl_idname = "photographer.fix_display_type"
    bl_label = "Fix Display Type"
    bl_description = "EEVEE requires the Optical Vignetting object Display Type to be Textured"
    bl_options = {'REGISTER', 'UNDO'}
