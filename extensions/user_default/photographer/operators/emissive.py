        ...
        for mat in bpy.data.materials:
            ...
            if mat.use_nodes and mat.users:
                ...
                if mat['em_nodes']:
                    ...
                    if prefs.hide_unlinked_emissive:
                        # Loop through all objects in the blend file
                        mat_objects = []
                        for obj in bpy.data.objects:
                            # Check if object uses the specified material
                            if obj.type in {'MESH','CURVE'} and mat.name in obj.data.materials:
                                mat_objects.append(obj)
                            # Check if the object is linked to any scene
                        for obj in mat_objects:
                            linked = any(scene_object == obj for scene in bpy.data.scenes for scene_object in scene.objects)
                            if linked:                            
...
class LIGHTMIXER_OT_CreateEmissive(bpy.types.Operator):
    ...
    bl_label = "Create Emissive Material"
    ...
    def execute(self, context):
        ...
        if self.type == 'EMISSION':
            ...
            if bpy.app.version >= (4,0,0):
                sh.inputs['Emission Color'].default_value = self.color
            else:
...
class LIGHTMIXER_OT_AssignEmissive(bpy.types.Operator):
    ...
    def poll(cls,context):
        active = context.active_object
        return active is not None and active.type in {'MESH','CURVE'} and (context.mode == 'EDIT_MESH' or active.select_get())
    ...
    def execute(self, context):
        ...
        if obj.type == 'MESH':
            ...
            # data.update() doesn't work for CURVE objects
            # Force viewport update by unhiding the object
            obj.hide_set(False)
            ...
            if not mat.get('em_nodes',False):
                ...
                for em_node in mat['em_nodes']:
                    ...
                    if em_controls_grp:
                        ...
                        if bpy.app.version >= (4,0,0):
                            col = em_controls_grp.interface.new_socket(name='Color Input', in_out='INPUT')
                            col.socket_type = 'NodeSocketColor'
                            str = em_controls_grp.interface.new_socket(name='Strength Input', in_out='INPUT')
                            str.socket_type = 'NodeSocketFloat'
                            rgb = em_controls_grp.interface.new_socket(name='Color Multiplier', in_out='INPUT')
                            rgb.socket_type = 'NodeSocketColor'
                            float = em_controls_grp.interface.new_socket(name='Strength Multiplier', in_out='INPUT')
                            float.socket_type = 'NodeSocketFloat'
                            em_controls_grp.interface.items_tree[0].default_value = (1.0,1.0,1.0,1.0)
                            em_controls_grp.interface.items_tree[1].default_value = 1.0
                            em_controls_grp.interface.items_tree[1].min_value = 0.0
                            em_controls_grp.interface.items_tree[2].default_value = (1.0,1.0,1.0,1.0)
                            em_controls_grp.interface.items_tree[3].default_value = 1.0
                            em_controls_grp.interface.items_tree[3].min_value = 0.0
                        else:
                        ...
                        if bpy.app.version >= (4,0,0):
                            em_controls_grp.interface.new_socket(socket_type='NodeSocketColor',name='Color', in_out='OUTPUT')
                            em_controls_grp.interface.new_socket(socket_type='NodeSocketFloat',name='Strength', in_out='OUTPUT')
                        else:
