...
class PHOTOGRAPHER_OT_LightgroupExpand(bpy.types.Operator):
    bl_idname = "photographer.lightgroup_expand"
    bl_label = "Expand Lightgroup"
    bl_description = "Show Lights that are part of this Lightgroup"

    lightgroup: bpy.props.StringProperty()
    emissive_mixer: bpy.props.BoolProperty()
    # expand_all: bpy.props.BoolProperty(default=False)

    def execute(self, context):
        lightgroup = context.view_layer.lightgroups[self.lightgroup]

        if self.emissive_mixer:
            exp = '_em_expand'
        else:
            exp = '_expand'

        # Lightgroup bpy_struct cannot save a property, adding it to the object
        lightgroup.id_data[self.lightgroup + exp] = not lightgroup.id_data.get(lightgroup.name + exp,True)
        # if self.expand_all:
        #     lightgroups=([lg for lg in context.view_layer.lightgroups if lg!=lightgroup])
        #     for lg in lightgroups:
        #         lg.id_data[self.lightgroup +'_expand'] = lightgroup.id_data[self.lightgroup +'_expand']
