...
def lens_panel_draw_header_preset(context,layout):
        ...
        if cam.type == 'ORTHO':
            ...
            if context.scene.render.engine == 'CYCLES':
                if bpy.app.version >= (4,0,0):
                    fisheye = cam.type == 'PANO' and cam.panorama_type == 'FISHEYE_EQUISOLID'
                else:
                    fisheye = cam.type == 'PANO' and cam.cycles.panorama_type == 'FISHEYE_EQUISOLID'
                if settings.fisheye or fisheye:
...
def lens_panel_draw(context,layout):
...
class PHOTOGRAPHER_PT_ViewPanel_Lens(Panel):
    ...
    def poll(cls, context):
        lpe = context.preferences.addons[base_package].preferences.lens_panel_enable
        return context.scene.camera is not None and context.scene.camera.type == 'CAMERA' and lpe

    def draw_header_preset(self, context):
        layout = self.layout
        lens_panel_draw_header_preset(context,layout)
        camera_presets.PHOTOGRAPHER_PT_LensPresets.draw_panel_header(layout)

    def draw(self, context):
        layout = self.layout
        lens_panel_draw(context,layout)