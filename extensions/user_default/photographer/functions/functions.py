...
# Focus picker
def raycast(context, event, cam_obj, continuous=False, coord=None):
    ...
    if unit_scale == 1:
        offset = 0.02 / unit_scale
        for c in cam_obj.children:
            if c.get("is_opt_vignetting", False):
                offset -= c.location.z * unit_scale
                break
        dir.normalize()
        org += (offset * dir)
    # If unit scale is not 1, simply disable DoF objects in Modals
...
def list_collections(context, excluded_collections=False):
    scene_colls = context.view_layer.layer_collection

    # Get names of collections and sort them
    if excluded_collections:
        collections = [c for c in traverse_tree(scene_colls)]
    else:
        collections = [c for c in traverse_tree(scene_colls) if not c.exclude]
...
def list_cameras(context, excluded_collections=False):
    cam_collections = []
    cameras = []

    camera_objects = [obj for obj in context.scene.collection.all_objects if (obj.type=='CAMERA' and obj.name !='DroneCamera')]
    cameras = [cam.name for cam in camera_objects]
    cameras.sort()

    collections = list_collections(context,excluded_collections)
    for coll in collections:
        coll_cams = [obj.name for obj in coll.objects if obj.type=='CAMERA']
        if coll_cams:
            cam_collections.append(coll)

    return cameras,cam_collections
...
def should_update(self,context,prop_enabled,post_effects=False):
    if context.scene.camera.type == 'CAMERA':
        ...
        pg_main_cam = context.scene.photographer.main_camera

        if pg_main_cam:
            if post_effects:
                pg_main_cam_settings = pg_main_cam.post_effects
            else:
...
def are_obj_transform_equal(obj1, obj2, location_tolerance=0.0001, rotation_tolerance=0.01):
    # Compare locations
    loc1 = obj1.location
    loc2 = obj2.location
    loc_equal = all(abs(a - b) < location_tolerance for a, b in zip(loc1, loc2))

    # Compare rotations
    rot1 = obj1.rotation_euler
    rot2 = obj2.rotation_euler
    rot_equal = all(abs(math.degrees(a - b)) < rotation_tolerance for a, b in zip(rot1, rot2))

    return loc_equal and rot_equal

def create_unique_name(base_name,data):
    data_names = [t.name for t in data]
    new_name = base_name
    counter = 1
    while new_name in data_names:
        new_name = f"{base_name}.{str(counter).zfill(3)}"
        counter += 1
    return new_name

def get_camera_object(camera):
    cam_obj = [o for o in bpy.data.objects if o.type == 'CAMERA' and o.data is camera.id_data]
    if cam_obj:
        return cam_obj[0]