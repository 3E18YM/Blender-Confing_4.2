...
from .operators.autofocus import dof_calculation
...
def dof_near_tracker(cam_matrix,tracker_matrix, fstop, fl, sw, sh):
    dist = distance_to_plane(cam_matrix,tracker_matrix)
    return dof_far(fstop, dist, fl, sw, sh) # Using dof_far on purpose, otherwise they are inverted with Tracker

def dof_far_tracker(cam_matrix,tracker_matrix, fstop, fl, sw, sh):
    dist = distance_to_plane(cam_matrix,tracker_matrix)
    return dof_near(fstop, dist, fl, sw, sh) # Using dof_near on purpose, otherwise they are inverted with Tracker
