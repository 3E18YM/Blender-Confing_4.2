...
def _cancel_redefine():
    ...
    if preserve:
        source_rig.data.pose_position = 'POSE'
        assign_armature_action(source_rig, bpy.data.actions.get(scn.source_action))
        target_rig.data.pose_position = 'POSE'
        
    else:
        source_rig_copy = get_object(scn.source_rig + "_copy")        
        source_rig.data.pose_position = 'POSE'
        assign_armature_action(source_rig, source_rig_copy.animation_data.action)
...
def _build_bones_list():
    ...
    def is_excluded_ctrl(ctrl_name):
        exclude_non_ctrl = ['c_p_', 
            'c_foot_fk_scale_fix', 'c_hand_fk_scale_fix',
            'c_toes_end', 'c_toes_track', 'c_foot_bank_', 'c_foot_heel', 'c_foot_roll', 'c_stretch_leg', 'c_stretch_leg_pin', 
...
def _retarget(self):
    ...
    if self.bind_only == False:
        ...
        for act_name in actions_to_bake:
            # set source action
            assign_armature_action(source_rig, bpy.data.actions.get(act_name))
        ...
        assign_armature_action(source_rig, bpy.data.actions.get(actions_to_bake[0]))
...
def update_in_place(self, context):
    ...
    if act:
        if scn.arp_retarget_in_place:
            ...
            assign_armature_action(rig, act_in_place)
            ...
            if act_base and "_IN_PLACE" in rig.animation_data.action.name:
                ...
                if act_in_place:
                    bpy.data.actions.remove(act_in_place)
                assign_armature_action(rig, act_base)
