...
class GeometryExportManager():
    ...
    def get_geometry_export_objects_by_export_type(self, export_type):
        if isinstance(export_type, MotionExportType):
            ...
            raise TypeError(_z("Value must MotionExportType or GeometryExportType enum."))


    def initialize(self):
        if self._is_initialized:
            raise Exception(_z("GeometryExportManager already initialized."))
        ...
        if motion_type == MotionExportType.STATIC:
            ...
            if not disable_warning:
                ...
                if previous_frame_bytes is not None and current_frame_bytes != previous_frame_bytes:
                    ...
                    error_reason = _z("Unknown")
                    if bl_object is not None:
                        if bl_object.flip_fluid.is_obstacle():
                            error_reason = _z("Obstacle object require mesh velocities to be computed for fluid interaction.")
                        elif bl_object.flip_fluid.is_inflow():
                            error_reason = _z("Inflow object 'Add Object Velocity to Inflow' option require mesh velocities to be computed for this feature.")
                        elif bl_object.flip_fluid.is_fluid():
                            error_reason = _z("Fluid object 'Add Object Velocity to Fluid' option require mesh velocities to be computed for this feature.")

                    errmsg = (_z("Warning: unable to export animated mesh '{}'. ").format(work_item.geometry_export_object.name) +
                             _z("Animated meshes must have the same number of vertices/triangles for each frame and must not change topology\n") +
