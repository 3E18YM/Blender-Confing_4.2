...
def draw_T88811_ui_warning(ui_box, preferences, feature_dict):
    ...
    row.alert = True
    row.label(text=_z("FLIP Fluids: Possible Render Crash Warning:"), icon="ERROR")
    column = ui_box.column(align=True)
    column.label(text=_z("A current bug in Blender may cause frequent")) 
    column.label(text=_z("render crashes or incorrect renders when")) 
    column.label(text=_z("using the following enabled features:"))
    column.separator()

    if feature_dict["attributes"]["surface"]:
        column.label(text=_z("Surface Attributes:"), icon="ERROR")
    ...
    if feature_dict["attributes"]["whitewater"]:
        column.label(text=_z("Whitewater Attributes:"), icon="ERROR")
    ...
    if feature_dict["fluidparticles"]:
        column.label(text=_z("Fluid Particle Features:"), icon="ERROR")
        column.label(text=get_enabled_features_string_T88811(feature_dict["fluidparticles"]), icon="DOT")
    if feature_dict["viscosity"]:
        column.label(text=_z("Viscosity Features:"), icon="ERROR")
        column.label(text=get_enabled_features_string_T88811(feature_dict["viscosity"]), icon="DOT")
    column.separator()

    column.label(text=_z("This bug can be prevented by rendering"), icon="INFO")
    column.label(text=_z("from the command line. See the cmd"))
    column.label(text=_z("rendering tools in the FLIP Fluids sidebar."))

    column.operator(
            "wm.url_open", 
            text=_z("Documentation: Command Line Tools"), 
            icon="URL"
    ...
    column.prop(preferences, "dismiss_T88811_crash_warning", text=_z("Do not show this warning again"))

def get_T88811_cmd_warning_string(feature_dict):
    warning = ""
    warning += "************************************************\n"
    warning += _z("FLIP Fluids: Possible Render Crash Warning\n\n")
    warning += _z("A current bug in Blender may cause frequent render crashes or incorrect renders when using the following enabled features:\n\n")

    if feature_dict["attributes"]["surface"]:
        warning += _z("* Surface Attributes:\n")
        warning += "    - " + get_enabled_features_string_T88811(feature_dict["attributes"]["surface"]) + "\n"
    if feature_dict["attributes"]["whitewater"]:
        warning += _z("* Whitewater Attributes:\n")
        warning += "    - " + get_enabled_features_string_T88811(feature_dict["attributes"]["whitewater"]) + "\n"
    if feature_dict["viscosity"]:
        warning += _z("* Viscosity Features:\n")
        warning += "    - " + get_enabled_features_string_T88811(feature_dict["viscosity"]) + "\n"

    warning += "\n"
    warning += _z("This bug can be prevented by rendering from the command line.\n")
    warning += _z("See the command line rendering tools in the FLIP Fluids sidebar.\n\n")
    warning += _z("Command Line Tools Documentation:\n    https://github.com/rlguy/Blender-FLIP-Fluids/wiki/Helper-Menu-Settings#command-line-tools\n")
    warning += _z("Bug Report T88811:\n    https://developer.blender.org/T88811\n")
    warning += "************************************************\n"
...
def draw_persistent_data_warning(ui_box, preferences):
    row = ui_box.row(align=True)
    row.alert = True
    row.label(text=_z("FLIP Fluids: Incompatible Render Option Warning:"), icon="ERROR")
    column = ui_box.column(align=True)
    column.label(text=_z("The Cycles 'Persistent Data' render option is not"))
    column.label(text=_z("compatible with the simulation meshes. This may"))
    column.label(text=_z("cause static renders, incorrect renders, or"))
    column.label(text=_z("render crashes."))
    column.separator()
    column.label(text=_z("This issue can be prevented by disabling"))
    column.label(text=_z("Render Properties > Performance > Persistent Data:"))
    ...
    column.label(text=_z("Or by rendering from the command line. See the"))
    column.label(text=_z("cmd rendering tools in the FLIP Fluids sidebar."))
    ...
    column.prop(preferences, "dismiss_persistent_data_render_warning", text=_z("Do not show this warning again"))


def get_persistent_data_warning_string():
    warning = ""
    warning += "************************************************\n"
    warning += _z("FLIP Fluids: Incompatible Render Option Warning\n\n")
    warning += _z("The Cycles 'Persistent Data' render option is not compatible with the simulation meshes. This may cause static renders, incorrect renders, or render crashes.\n\n")
    warning += _z("This issue can be prevented by disabling the 'Render Properties > Performance > Persistent Data' option or by rendering from the command line.\n")
    warning += _z("See the command line rendering tools in the FLIP Fluids sidebar.\n\n")
    warning += _z("Command Line Tools Documentation:\n    https://github.com/rlguy/Blender-FLIP-Fluids/wiki/Helper-Menu-Settings#command-line-tools\n")
    warning += "************************************************\n"
    return warning
