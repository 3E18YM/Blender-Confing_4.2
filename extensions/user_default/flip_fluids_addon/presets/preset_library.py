...
def get_deletable_package_enums(self, context):
    ...
    enums = []
    enums.append(('DELETE_PACKAGE_NONE', _z("None"), "None", 0))
    for info in info_list:
        e = (info['identifier'], info['name'], info['description'], info['uid'])
        enums.append(e)
    return enums
...
def get_exportable_package_enums(self, context):
    ...
    enums = []
    enums.append(('EXPORT_PACKAGE_NONE', _z("None"), "None", 0))
    for info in info_list:
        e = (info['identifier'], info['name'], info['description'], info['uid'])
        enums.append(e)
    return enums
...
def get_current_package_preset_enums(self, context):
    ...
    enums.append(('PRESET_NONE', _z("None"), "None", 0))
...
def get_deletable_preset_enums(self, context):
    ...
    enums = []
    enums.append(('DELETE_PRESET_NONE', _z("None"), "None", 0))
...
def restore_default_settings(domain_properties):
    ...
    default_preset_file = os.path.join(sys_path, "default.preset")
    if not os.path.isfile(default_preset_file):
        return _z("Missing default preset file: <{}>").format(default_preset_file)

    with open(default_preset_file, 'r', encoding='utf-8') as f:
        try:
            default_data = json.loads(f.read())
        except:
            return _z("Unable to decode default preset file: <{}>").format(default_preset_file)
...
def create_new_user_package(package_info_dict):
    ...
    if not identifier:
        return _z("Unable to create unique package identifier")
    ...
    if os.path.exists(package_path):
        err = _z("Unable to create package directory. Directory already exists <{}>").format(package_path)
        return err

    try:
        os.makedirs(package_path)
    except:
        return _z("Unable to create package directory")
...
def delete_package(identifier):
    ...
    try:
        ...
        return _z("Unable to delete package directory <{}>").format(package_path)
    __initialize_package_info_list()
...
def create_new_user_preset(preset_info_dict):
    ...
    if not identifier:
        return _z("Unable to create unique preset identifier")
    ...
    package_path = package_info["path"]
    if not package_path:
        return _z("Unable to locate package directory: <{}>").format(preset_info_dict["package"])
    ...
    if os.path.exists(preset_directory):
        err = _z("Unable to create preset directory. Directory already exists <{}>").format(preset_directory)
        return err
    else:
    ...
    if preset_info_dict["icon"]:
        ...
        if not icon_imgdata:
            return _z("Unable to locate icon: <{}>").format(preset_info_dict["icon"])
...
def edit_user_preset(preset_info_dict):
    ...
    try:
        ...
        return _z("Unable to delete original preset directory <{}>").format(original_preset_path)
...
def delete_preset(identifier):
    ...
    try:
        ...
        return _z("Unable to delete preset directory <{}>").format(preset_path)
    __initialize_preset_info_list()
...
def export_package(identifier, filepath, create_directory=True):
    ...
    if create_directory and not os.path.isdir(directory):
        try:
            ...
            return _z("Unable to create file: <{}>. Please choose another filepath. This directory may be invalid or you may not have permission to write in this directory.").format(filepath)
    if not os.path.isdir(directory):
        return _z("Directory does not exist <{}>").format(filepath)

    info = package_identifier_to_info(identifier)
    if not info:
        return _z("Package does not exist <{}>").format(identifier)
    package_path = info['path']
    
    try:
        shutil.make_archive(filepath.replace(".zip", ""), 'zip', package_path)
    except:
        return _z("Error creating package archive")
...
def import_package(package_filepath):
    ...
    if os.path.isdir(package_path):
        return _z("Package already exists <{}>").format(package_path)

    try:
        shutil.unpack_archive(package_filepath, package_path, 'zip')
    except:
        return _z("Unable to unpack package")
...
def decode_package_zipfile(filepath, dst_data):
    if not os.path.isfile(filepath):
        return _z("Package file does not exist <{}>").format(filepath)
        ...
        for f in preset_data_files:
            with zfile.open(f, 'r', encoding='utf-8') as info_file:
                try:
                    ...
                    return _z("Unable to decode preset info file <{}>").format(f)
