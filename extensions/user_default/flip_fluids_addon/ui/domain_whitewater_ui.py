...
def _draw_geometry_attributes_menu(self, context):
    ...
    row.label(text=_z("Whitewater Attributes:"))
    ...
    if wprops.geometry_attributes_expanded:
        ...
        if not vcu.is_blender_31():
            ...
            column.label(text=_z("Geometry attribute features for whitewater are only available in"), icon='ERROR')
            column.label(text=_z("Blender 3.1 or later"), icon='ERROR')
            return
        ...
        if not vcu.is_blender_31():
            ...
            row.enabled = False
            row.alignment = 'RIGHT'
            row.label(text=_z("(Blender 3.1 or later required)"))
            return
        if not prefs.is_developer_tools_enabled():
            return
        row = row.row(align=True)
        row.alignment = 'RIGHT'
        row.prop(wprops, "enable_velocity_vector_attribute", text=_z("Velocity"))
        row.prop(wprops, "enable_id_attribute", text="ID")
        row.prop(wprops, "enable_lifetime_attribute", text=_z("Lifetime"))
...
class FLIPFLUID_PT_DomainTypeWhitewaterPanel(bpy.types.Panel):
    ...
    bl_label = _z("FLIP Fluid Whitewater")
    bl_description = "FLIP Fluid Whitewater"
    ...
    def draw(self, context):
        ...
        if show_documentation:
            ...
            column.operator(
                "wm.url_open", 
                text=_z("Whitewater Documentation"), 
                icon="WORLD"
            ).url = "https://github.com/rlguy/Blender-FLIP-Fluids/wiki/Domain-Whitewater-Settings"
            column.operator(
                "wm.url_open", 
                text=_z("Simulation not generating enough whitewater"), 
                icon="WORLD"
            ...
            column.operator(
                "wm.url_open", 
                text=_z("Whitewater particles are rendered too large"), 
                icon="WORLD"
            ...
            column.operator(
                "wm.url_open", 
                text=_z("Whitewater particles are not rendered in preview render"), 
                icon="WORLD"
        ...
        row.label(text=_z("Settings View Mode:"))
        ...
        row.label(text=_z("Whitewater Particles:"))

        if not wprops.whitewater_simulation_particles_expanded:
            info_text = ""
            enabled_particles = []
            if wprops.enable_foam:
                enabled_particles.append(_z("Foam"))
            if wprops.enable_bubbles:
                enabled_particles.append(_z("Bubble"))
            if wprops.enable_spray:
                enabled_particles.append(_z("Spray"))
            if wprops.enable_dust:
                enabled_particles.append(_z("Dust"))
        ...
        row.label(text=_z("Emitter Settings:"))
        ...
        if wprops.emitter_settings_expanded:
            ...
            column = box.column(align=True)
            column.enabled = wprops.enable_dust
            column.prop(wprops, "enable_dust_emission_near_boundary", text=_z("Enable dust emission near domain floor"))
        ...
        row.label(text=_z("Particle Behavior Settings:"))
        ...
        if wprops.particle_settings_expanded:
            ...
            row = column.row()
            row.prop(wprops, "foam_advection_strength", text=_z("Advection Strength"), slider=True)

            if show_advanced_whitewater:
                row = column.row()
                row.alert = highlight_advanced
                row.prop(wprops, "foam_layer_depth", text=_z("Depth"), slider=True)

                row = column.row()
                row.alert = highlight_advanced
                row.prop(wprops, "foam_layer_offset", text=_z("Offset"), slider=True)

            column = box.column(align=True)
            column.label(text=_z("Bubble:"))
            column.prop(wprops, "bubble_drag_coefficient", text=_z("Drag Coefficient"), slider=True)
            column.prop(wprops, "bubble_bouyancy_coefficient", text=_z("Buoyancy Coefficient"))

            column = box.column(align=True)
            column.label(text=_z("Spray:"))
            column.prop(wprops, "spray_drag_coefficient", text=_z("Drag Coefficient"), slider=True)

            column = box.column(align=True)
            column.enabled = wprops.enable_dust
            column.label(text=_z("Dust:"))
            column.prop(wprops, "dust_drag_coefficient", text=_z("Drag Coefficient"), slider=True)
            column.prop(wprops, "dust_bouyancy_coefficient", text=_z("Buoyancy Coefficient"))

            column = box.column(align=True)
            split = column.split()
            column = split.column(align=True)
            column.label(text=_z("Lifespan:"))
            column.prop(wprops.min_max_whitewater_lifespan, "value_min", text=_z("Min"))
            column.prop(wprops.min_max_whitewater_lifespan, "value_max", text=_z("Max"))
            column.prop(wprops, "whitewater_lifespan_variance", text=_z("Variance"))

            column = split.column(align=True)
            column.label(text=_z("Lifespan Modifiers:"))
            column.prop(wprops, "foam_lifespan_modifier", text=_z("Foam"))
            column.prop(wprops, "bubble_lifespan_modifier", text=_z("Bubble"))
            column.prop(wprops, "spray_lifespan_modifier", text=_z("Spray"))
            column = column.column(align=True)
            column.enabled = wprops.enable_dust
            column.prop(wprops, "dust_lifespan_modifier", text=_z("Dust"))
        ...
        row.label(text=_z("Obstacle Influence Settings:"))
        ...
        if wprops.obstacle_settings_expanded:
            ...
            indent_str = 5 * " "
            column.label(text=_z("Obstacle Object Influence:"))
            if len(obstacle_objects) == 0:
                column.label(text=indent_str + _z("No obstacle objects found..."))
            else:
                ...
                for ob in obstacle_objects:
                    ...
                    row.alignment = 'RIGHT'
                    row.prop(pgroup, "whitewater_influence", text=_z("influence"))
                    row.prop(pgroup, "dust_emission_strength", text=_z("dust emission"))
