...
class FLIPFLUID_PT_HelperPanelMain(bpy.types.Panel):
    bl_label = _z("Simulation Setup")
    bl_description = "Simulation Setup"
    bl_category = _z("FLIP Fluids")
    ...
    def draw_simulation_setup_panel(self, context):
        ...
        row.label(text=_z("Bake Simulation:"))
        ...
        if hprops.bake_simulation_expanded:
            if context.scene.flip_fluid.is_domain_object_set():
                ...
                if not is_saved:
                    ...
                    row.alignment = 'LEFT'
                    row.alert = True
                    row.label(text=_z("Unsaved File"))
                    row = row.row(align=True)
                    row.alignment = 'RIGHT'
                    row.alert = False
                    row.operator("flip_fluid_operators.helper_save_blend_file", icon='FILE_TICK', text=_z("Save"))
                ...
                resolution_text = _z("Resolution")
                if dprops.simulation.lock_cell_size:
                    resolution_text += _z(" (voxel size locked)")
                ...
                column_left.label(text=_z("Multithreading:"))
                ...
                column.label(text=_z("Cache Directory:"))
                ...
                column = box.column(align=True)
                column.label(text=_z("Render Output:"))
                row = column.row(align=True)
                ...
                column = box.column(align=True)
                column.label(text=_z("Cache and Render Output:"))
                row = column.row(align=True)
                row.operator("flip_fluid_operators.increase_decrease_cache_render_version", text=_z("Decrease Version"), icon="REMOVE").increment_mode = "DECREASE"
                row.operator("flip_fluid_operators.increase_decrease_cache_render_version", text=_z("Increase Version"), icon="ADD").increment_mode = "INCREASE"

            else:
                box.label(text=_z("Please create a domain object"))
        ...
        row.label(text=_z("Prepare Geometry Tools:"))

        if hprops.prepare_geometry_tools_expanded:
            column = box.column(align=True)

            if not vcu.is_blender_31():
                column.label(text=_z("Blender 3.1 or later required"))
            ...
            if is_developer_mode:
                ...
                active_collection_name = _z("No Collection Selected")
                if active_collection is not None:
                    is_active_collection_selected = True
                    active_collection_name = active_collection.name

                options_box = column.box()
                options_box.label(text=_z("Remesh Options:"), icon='TOOL_SETTINGS')
                ...
                options_column.label(text=_z("Active collection will be remeshed:"))
                ...
                warn_column.label(text=_z("     This feature is affected by a current bug in Blender."), icon='ERROR')
                warn_column.label(text=_z("     The Developer Tools option must be enabled in preferences"))
                warn_column.label(text=_z("     to use this feature."))
                warn_column.separator()
                warn_column.prop(prefs, "enable_developer_tools", text=_z("Enable Developer Tools in Preferences"))
                warn_column.separator()
                warn_column.operator(
                    "wm.url_open", 
                    text=_z("Important Info and Limitations"), 
                    icon="WORLD"
        ...
        row.label(text=_z("Add / Remove Objects:"))
        ...
        if hprops.add_remove_objects_expanded:
            ...
            column = box.column(align=True)
            column.operator("flip_fluid_operators.helper_remove_objects", text=_z("Remove"))
            column = box.column()
            column.operator("flip_fluid_operators.helper_delete_domain", text=_z("Delete Domain"), icon='X')

            column = box.column()
            column.label(text=_z("Object Display:"))
            row = column.row(align=True)
            row.operator(
        ...
        row.label(text=_z("Select Objects:"))
        if not hprops.quick_select_expanded:
            row.operator("flip_fluid_operators.helper_select_domain", text=_z("Domain"), icon="MESH_GRID")

        if hprops.quick_select_expanded:
            column = box.column(align=True)
            column.operator("flip_fluid_operators.helper_select_domain", text=_z("Domain"), icon="MESH_GRID")
            ...
            column = box.column(align=True)
            row = column.row(align=True)
            row.operator("flip_fluid_operators.helper_select_surface", text=_z("Fluid Surface"))
            row.operator("flip_fluid_operators.helper_select_fluid_particles", text=_z("Fluid Particles"))
            row = column.row(align=True)
            row.operator("flip_fluid_operators.helper_select_foam", text=_z("Foam"))
            row.operator("flip_fluid_operators.helper_select_bubble", text=_z("Bubble"))
            row.operator("flip_fluid_operators.helper_select_spray", text=_z("Spray"))
            row.operator("flip_fluid_operators.helper_select_dust", text=_z("Dust"))
        ...
        if vcu.is_blender_28():
            ...
            row.label(text=_z("Organize Outliner:"))
            if not hprops.outliner_organization_expanded:
                row.operator("flip_fluid_operators.helper_organize_outliner", text=_z("Organize"), icon="GROUP")

            if hprops.outliner_organization_expanded:
                column = box.column(align=True)
                column.operator("flip_fluid_operators.helper_organize_outliner", text=_z("FLIP Objects to Collections"))
                column.operator("flip_fluid_operators.helper_undo_organize_outliner", text=_z("Unlink FLIP Object Collections"))

                column = box.column(align=True)
                column.operator("flip_fluid_operators.helper_separate_flip_meshes")
                column.operator("flip_fluid_operators.helper_undo_separate_flip_meshes", text=_z("Unlink FLIP Mesh Collections"))
        ...
        row.label(text=_z("Command Line Tools:"))

        if hprops.command_line_tools_expanded:
            row = box.row(align=True)
            row.alignment = 'LEFT'
            row.label(text=_z("Save before running CMD operators:"))
            row.operator("flip_fluid_operators.helper_save_blend_file", icon='FILE_TICK', text=_z("Save")).save_as_blend_file = False
            ...
            if hprops.display_compositing_tools_in_ui or override_preferences:
                ...
                row.label(text=_z("Compositing Tools:"))

                if not hprops.command_line_render_passes_expanded:
                    row.operator("flip_fluid_operators.helper_cmd_render_pass_animation", text=_z("Launch Render Passes"))
                ...
                if hprops.command_line_render_passes_expanded:
                    ...
                    if context.scene.render.engine == 'CYCLES':
                        row.prop(context.scene.render, "film_transparent", text=_z("Film Transparency"))
                    ...
                    row = column.row(align=True)
                    row.operator("flip_fluid_operators.reset_passes_settings", text=_z("Reset Settings"), icon='FILE_REFRESH')
                    
                    # Add some objects to list
                    row = subbox.row()
                    row.label(text=_z("Objects to render (no fluid objects):"))
                    row = subbox.row()
                    ...
                    row.operator("flip_fluid_operators.add_camera_screen", text=_z("Add CameraScreen"), icon='IMAGE_BACKGROUND')
                    ...
                    if camera and camera.type == 'CAMERA':
                        column = subbox.column()
                        row = column.row(align=True)
                        row.prop(camera.data, "show_background_images", text=_z("Show Background Image"))
                        ...
                        if camera.data.show_background_images:
                            ...
                            if bg_image and bg_image.image:
                                row.prop(bg_image, "alpha", text=_z("Opacity"))

                    column = subbox.column()
                    row = column.row(align=True)
                    row.operator("flip_fluid_operators.helper_fix_compositingtextures", text=_z("Fix Textures"))
            ...
            row.label(text=_z("Render Animation:"))
            ...
            row.label(text=_z("Render Frame:"))
            ...
            if installation_utils.is_turbo_tools_addon_enabled():
                ...
                row.label(text=_z("Turbo Tools Command Line Render:"))
                ...
                if hprops.command_line_render_turbo_tools_expanded:
                    ...
                    row.label(text=_z("Turbo Tools Addon Detected"))

                    column = subbox.column(align=True)
                    row = column.row(align=True)
                    row.operator("flip_fluid_operators.helper_command_line_render", text=_z("Render Animation")).use_turbo_tools = True
                    row.operator("flip_fluid_operators.helper_command_line_render_to_clipboard", text="", icon='COPYDOWN').use_turbo_tools = True
                    row = column.row(align=True)
                    row.operator("flip_fluid_operators.helper_command_line_render_frame", text=_z("Render Frame")).use_turbo_tools = True
            ...
            row.label(text=_z("Alembic Export:"))
            ...
            if hprops.command_line_alembic_export_expanded:
                ...
                column.label(text=_z("Mesh and Data Export:"))
                row = column.row(align=True)
                row.alignment = 'LEFT'
                ...
                column.label(text=_z("Frame Range:"))
                row = column.row()
                ...
                column.separator()
                column.label(text=_z("Alembic Output:"))
        ...
        row.label(text=_z("Geometry Node Tools:"))
        ...
        if hprops.geometry_node_tools_expanded:
            ...
            row = column.row(align=True)
            row.label(text=_z("Toggle Motion Blur Rendering:"))
            row = row.row(align=True)
            row.alignment = 'RIGHT'
        ...
        row.label(text=_z("Measure Object Speed Tool:"))
        ...
        if hprops.object_speed_measurement_tools_expanded:
            ...
            if not hprops.is_translation_data_available:
                column.label(text=_z("No Speed Data Available"))
                column.label(text=_z("Select an object and use the above operator"))
            else:
                try:
                    world_scale = 1.0
                    time_scale = 1.0
                ...
                header_text = _z("Name: ") + hprops.translation_data_object_name + " | "
                header_text += _z("Frame: ") + str(hprops.translation_data_object_frame) + " | "
                header_text += _z("Verts: ") + str(hprops.translation_data_object_vertices) + " | "
                header_text += _z("Time: ") + str(hprops.translation_data_object_compute_time) + "ms"
                ...
                column1.label(text=_z("Center Speed:"))
                column1.label(text=_z("Min Vertex Speed:"))
                column1.label(text=_z("Max Vertex Speed:"))
                ...
                footer_text = _z("World Scale: ") + '{0:.3f}'.format(world_scale) + " | "
                footer_text += _z("Time Scale: ") + '{0:.3f}'.format(time_scale) + " | "
                footer_text += _z("FPS: ") + '{0:.2f}'.format(frame_rate)
                column.separator()
        ...
        row.label(text=_z("Beginner Tips:"))
        ...
        if hprops.beginner_tools_expanded:
            ...
            if preferences.show_documentation_in_ui:
                column.operator(
                    "wm.url_open", 
                    text=_z("Helper Menu Documentation"), 
                    icon="WORLD"
    ...
    def draw_disable_addon_submenu(self, context):
        ...
        if is_addon_disabled:
            row.label(text=_z("Enable Addon:"))
            row.alert = True
            row.label(text=_z("FLIP Fluids is disabled"))
            row.label(text="", icon='X')
        else:
            row.label(text=_z("Disable Addon:"))
            row.label(text=_z("FLIP Fluids is enabled"))
    ...
    def draw_header_preset(self, context): #draw_header_preset
        layout = self.layout
        row= layout.row()

        row.operator('wm.url_open', text = '', icon = 'QUESTION').url = "https://www.bilibili.com/video/BV17k4y1g7PN/"
    ...
    def draw(self, context):
        ...
        layout = self.layout
        box = layout.box()
        box.prop(preferences, "language", expand=False)

        if not installation_utils.is_installation_complete():
            box = self.layout.box()
            box.label(text=_z("IMPORTANT: Please Complete Installation"), icon="ERROR")
            box.label(text=_z("Click here to complete installation of the FLIP Fluids Addon:"))
...
class FLIPFLUID_PT_HelperPanelDisplay(bpy.types.Panel):
    bl_label = _z("Display and Playback")
    bl_description = "Display and Playback"
    bl_category = _z("FLIP Fluids")
    bl_space_type = 'VIEW_3D'
    if vcu.is_blender_28():
        bl_region_type = 'UI'
    ...
    def draw(self, context):
        ...
        if dprops is None:
            self.layout.label(text=_z("Please create a domain object"))
        ...
        row.label(text=_z("Quick Viewport Display:"))
        ...
        if hprops.quick_viewport_display_expanded:
            ...
            column = box.column(align=True)
            row = column.row(align=True)
            row.label(text=_z("Simulation Visibility:"))
            if not scene_props.show_viewport:
                row = row.row(align=True)
                row.alert = True
                row.label(text=_z("Disabled in Viewport"), icon="CANCEL")

            column.prop(scene_props, "show_viewport", text=_z("Show In Viewport"), icon="RESTRICT_VIEW_OFF")

            column.label(text=_z("Surface Display:"))
            row = column.row(align=True)
            row.prop(rprops, "viewport_display", expand=True)

            column.label(text=_z("Fluid Particle Display:"))
            row = column.row(align=True)
            row.prop(rprops, "fluid_particle_viewport_display", expand=True)


            column.separator()
            column.label(text=_z("Whitewater Display:"))
            row = column.row(align=True)
            row.prop(rprops, "whitewater_viewport_display", expand=True)

            column.separator()
            column.label(text=_z("Load Frame:"))
            column.operator("flip_fluid_operators.reload_frame", text=_z("Reload Frame"))
        ...
        if hprops.simulation_playback_expanded:
            ...
            if rprops.simulation_playback_mode == 'PLAYBACK_MODE_TIMELINE':
                ...
                left_column.prop(rprops, "override_frame", text=_z("Frame"))
                ...
                split = vcu.ui_split(column, factor=0.5, align=True)
                left_column = split.column(align=True)
                left_column.prop(rprops, "hold_frame_number", text=_z("Frame"))

                right_column = split.column(align=True)
                right_column.alert = True
                right_column.operator("flip_fluid_operators.free_unheld_cache_files", 
                                      text=_z("Delete Other Cache Files"))

            column = box.column(align=True)
            column.separator()
            column.prop(hprops, "playback_frame_offset")
            column.label(text=_z("Current Simulation Frame:     ") + str(render.get_current_simulation_frame()))
...
class FLIPFLUID_PT_HelperTechnicalSupport(bpy.types.Panel):
    bl_label = _z("Technical Support Tools")
    bl_description = "Technical Support Tools"
    bl_category = _z("FLIP Fluids")
    bl_space_type = 'VIEW_3D'
