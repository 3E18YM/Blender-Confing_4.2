...
class FLIPFLUID_PT_ForceFieldTypePanel(bpy.types.Panel):
    ...
    bl_label = _z("FLIP Fluid")
    bl_description = "FLIP Fluid"
    ...
    def draw(self, context):
        ...
        if show_disabled_in_viewport_warning and obj.hide_viewport:
            ...
            row.label(text=_z("Object is currently disabled in the viewport"))
            row.label(text="", icon="RESTRICT_VIEW_ON")
            column.label(text=_z("This object will not be included within the simulation"))

        column = self.layout.column()
        column.prop(obj_props, "object_type")

        column = self.layout.column()
        column.prop(force_field_props, "force_field_type", text=_z("Mode"))

        if obj.type == 'CURVE' and force_field_props.force_field_type != 'FORCE_FIELD_TYPE_CURVE':
            column = self.layout.column()
            column.label(text=_z("Curve objects can only be set as a Curve Guide Force Field"))
            return

        if obj.type == 'EMPTY' and force_field_props.force_field_type != 'FORCE_FIELD_TYPE_POINT':
            column = self.layout.column()
            column.label(text=_z("Empty objects can only be set as a Point Force Field"))
            return

        if obj.type == 'MESH' and force_field_props.force_field_type == 'FORCE_FIELD_TYPE_CURVE':
            column = self.layout.column()
            column.label(text=_z("Mesh objects cannot be used as a Curve Guide Force Field"))
            column.label(text=_z("Curve Guide Force Fields only support Curve objects"))
            return

        elif force_field_props.force_field_type == 'FORCE_FIELD_TYPE_VORTEX':
            column = self.layout.column()
            column.label(text=_z("Vortex force is not yet available"))
            column.label(text=_z("Feature implementation in progress"))
            return
        elif force_field_props.force_field_type == 'FORCE_FIELD_TYPE_TURBULENCE':
            column = self.layout.column()
            column.label(text=_z("Turbulence guided force is not yet available"))
            column.label(text=_z("Feature implementation in progress"))
            return
        elif force_field_props.force_field_type == 'FORCE_FIELD_TYPE_PROGRAMMABLE':
            column = self.layout.column()
            column.label(text=_z("Programmable guided force is not yet available"))
            column.label(text=_z("Feature implementation in progress"))
            return
        elif force_field_props.force_field_type == 'FORCE_FIELD_TYPE_OTHER':
            column = self.layout.column()
            column.label(text=_z("More force field modes are in development"))
            column.label(text=_z("Try out our experimental builds for the latest features!"))
            column.operator(
                "wm.url_open", 
                text=_z("Experimental Builds"), 
                icon="WORLD"
        ...
        if show_documentation:
            ...
            column.operator(
                "wm.url_open", 
                text=_z("Force Field Object Settings"), 
                icon="WORLD"
            ...
            column.operator(
                "wm.url_open", 
                text=_z("Force Field Example Scenes"), 
                icon="WORLD"
        ...
        strength_text = _z("Strength")
        if force_field_props.force_field_type == 'FORCE_FIELD_TYPE_CURVE':
            strength_text = _z("Attraction Strength")

        box = self.layout.box()
        box.label(text=_z("Field Strength and Falloff:"))
        column = box.column(align=True)
        ...
        if force_field_props.force_field_type == 'FORCE_FIELD_TYPE_SURFACE':
            ...
            row.label(text=_z("Enabled Sides:"))
        ...
        box = self.layout.box()
        box.label(text=_z("Antigravity"))
        column = box.column(align=True)

        if force_field_props.force_field_type == 'FORCE_FIELD_TYPE_POINT':
            column.prop(force_field_props, "gravity_scale_point", slider=True)
            column.prop(force_field_props, "gravity_scale_width_point", text=_z("Width"), slider=True)
            ...
            column.prop(force_field_props, "gravity_scale_width_surface", text=_z("Width"), slider=True)
            ...
            column.prop(force_field_props, "gravity_scale_width_volume", text=_z("Width"), slider=True)
            ...
            column.prop(force_field_props, "gravity_scale_width_curve", text=_z("Width"), slider=True)
        
        box = self.layout.box()
        box.label(text=_z("Mesh Data Export:"))
        column = box.column(align=True)
        ...
        if is_hint_enabled and not force_field_props.export_animated_mesh and is_child_object:
            ...
            row.label(text=_z("←Hint: export option may be required"))
