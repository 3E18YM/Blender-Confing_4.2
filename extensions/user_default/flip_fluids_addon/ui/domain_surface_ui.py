...
def _draw_geometry_attributes_menu(self, context):
    ...
    row.label(text=_z("Surface Attributes:"))
    ...
    if sprops.geometry_attributes_expanded:
        ...
        if not vcu.is_blender_293():
            ...
            column.label(text=_z("Geometry attribute features for the fluid surface are only available in"), icon='ERROR')
            column.label(text=_z("Blender 2.93 or later. Blender 3.1 or later recommended."), icon='ERROR')
            return
        ...
        if is_preview_mode_enabled and is_attributes_enabled:
            ...
            row.label(text=_z("Warning: Surface attributes will not be loaded in mesh Preview Mode"))
        ...
        row.label(text=_z("Velocity Based Attributes:"))
        ...
        if sprops.velocity_attributes_expanded:
            ...
            column_left.prop(sprops, "enable_velocity_vector_attribute", text=_z("Velocity Attributes"))
            ...
            column.prop(sprops, "enable_speed_attribute", text=_z("Speed Attributes"))
            column.prop(sprops, "enable_vorticity_vector_attribute", text=_z("Vorticity Attributes"))
            column.separator()
            column.operator("flip_fluid_operators.helper_initialize_motion_blur")
        else:
            row = row.row(align=True)
            row.alignment = 'RIGHT'
            row.prop(sprops, "enable_velocity_vector_attribute", text=_z("Velocity"))
            row.prop(sprops, "enable_speed_attribute", text=_z("Speed"))
            row.prop(sprops, "enable_vorticity_vector_attribute", text=_z("Vorticity"))
        ...
        row.label(text=_z("Color and Mixing Attributes:"))
        ...
        if sprops.color_attributes_expanded:
            ...
            column_left = split.column(align=True)
            column_right = split.column(align=True)
            column_left.prop(sprops, "enable_color_attribute", text=_z("Color Attributes"))
            if sprops.show_smoothing_radius_in_ui:
                column_right.prop(sprops, "color_attribute_radius", text=_z("Smoothing"), slider=True)
            ...
            column_left.prop(sprops, "enable_color_attribute_mixing", text=_z("Enable Mixing"))
            column_right.enabled = sprops.enable_color_attribute and sprops.enable_color_attribute_mixing
            column_right.prop(sprops, "color_attribute_mixing_rate", text=_z("Mix Rate"), slider=True)
            column_right.prop(sprops, "color_attribute_mixing_radius", text=_z("Mix Radius"), slider=True)

            column = subbox.column(align=True)
            column.enabled = sprops.enable_color_attribute and sprops.enable_color_attribute_mixing
            column.label(text=_z("Mixing Mode:"))
            ...
            if sprops.color_attribute_mixing_mode == 'COLOR_MIXING_MODE_MIXBOX':
                if not installation_utils.is_mixbox_supported():
                    column.label(text=_z("Mixbox feature is not supported"), icon="ERROR")
                    column.label(text=_z("in this version of the FLIP Fluids Addon"), icon="ERROR")

                if installation_utils.is_mixbox_supported():
                    if installation_utils.is_mixbox_installation_complete():
                        column.label(text=_z("Mixbox Plugin Status: Installed"), icon="CHECKMARK")
                    else:
                        column.label(text=_z("Install the Mixbox plugin in the"), icon="INFO")
                        column.label(text=_z("FLIP Fluids Addon preferences"), icon="INFO")
                        column.operator(
                                "flip_fluid_operators.open_preferences", 
                                text=_z("Open Preferences"), icon="PREFERENCES"
                                ).view_mode = 'PREFERENCES_MENU_VIEW_MIXBOX'
        else:
            row = row.row(align=True)
            row.alignment = 'RIGHT'
            row.prop(sprops, "enable_color_attribute", text=_z("Color"))
            row = row.row(align=True)
            row.alignment = 'RIGHT'
            row.enabled = sprops.enable_color_attribute
            row.prop(sprops, "enable_color_attribute_mixing", text=_z("Mixing"))
        ...
        row.label(text=_z("Other Attributes:"))

        if sprops.other_attributes_expanded:
            column = subbox.column(align=True)
            row = column.row(align=True)
            row.prop(sprops, "enable_age_attribute", text=_z("Age Attributes"))
            if sprops.show_smoothing_radius_in_ui:
                row.prop(sprops, "age_attribute_radius", text=_z("Smoothing"), slider=True)
            else:
                row.label(text="")
            row = column.row(align=True)
            row.prop(sprops, "enable_lifetime_attribute", text=_z("Lifetime Attributes"))
            row.prop(sprops, "lifetime_attribute_death_time")
            if sprops.show_smoothing_radius_in_ui:
                row.prop(sprops, "lifetime_attribute_radius", text=_z("Smoothing"), slider=True)
            row = column.row(align=True)
            row.prop(sprops, "enable_whitewater_proximity_attribute", text=_z("Whitewater Proximity Attributes"))
            if sprops.show_smoothing_radius_in_ui:
                row.prop(sprops, "whitewater_proximity_attribute_radius", text=_z("Smoothing"), slider=True)
            column.prop(sprops, "enable_source_id_attribute", text=_z("Source ID Attributes"))
        else:
            row = row.row(align=True)
            row.alignment = 'RIGHT'
            row.prop(sprops, "enable_age_attribute", text=_z("Age"))
            row.prop(sprops, "enable_lifetime_attribute", text=_z("Life"))
            row.prop(sprops, "enable_whitewater_proximity_attribute", text=_z("WW Prox."))
            row.prop(sprops, "enable_source_id_attribute", text=_z("Source ID"))

        if show_documentation:
            column = box.column(align=True)
            column.operator(
                "wm.url_open", 
                text=_z("Domain Attributes Documentation"), 
                icon="WORLD"
            ).url = "https://github.com/rlguy/Blender-FLIP-Fluids/wiki/Domain-Attributes-and-Data-Settings"
            column.operator(
                "wm.url_open", 
                text=_z("Attributes and Motion Blur Example Scenes"), 
                icon="WORLD"
...
class FLIPFLUID_PT_DomainTypeFluidSurfacePanel(bpy.types.Panel):
    ...
    bl_label = _z("FLIP Fluid Surface")
    bl_description = "FLIP Fluid Surface"
    ...
    def draw(self, context):
        ...
        row.label(text=_z("Surface Mesh:"))

        if not sprops.surface_mesh_expanded:
            info_text = _z("Subdivisions ") + str(sprops.subdivisions) + "  /  "
            info_text += _z("Scale") + " {:.2f}".format(sprops.particle_scale)
            row = row.row(align=True)
            row.alignment = 'RIGHT'
        ...
        if sprops.meshing_volume_expanded:
            ...
            column_right.prop_search(sprops, "meshing_volume_object", object_collection, search_group, text=_z("Object"))
        ...
        row.label(text=_z("Meshing Against Boundary:"))

        if not sprops.meshing_against_boundary_expanded:
            row = row.row(align=True)
            row.alignment = 'RIGHT'
            row.prop(sprops, "remove_mesh_near_domain", text=_z("Remove"))
        ...
        row.label(text=_z("Meshing Against Obstacles:"))

        if not sprops.meshing_against_obstacles_expanded:
            row = row.row(align=True)
            row.alignment = 'RIGHT'
            row.prop(sprops, "enable_meshing_offset", text=_z("Enable  "))
        ...
        if sprops.meshing_against_obstacles_expanded:
            ...
            if show_documentation:
                ...
                column.operator(
                    "wm.url_open", 
                    text=_z("Which meshing offset mode to use?"), 
                    icon="WORLD"
