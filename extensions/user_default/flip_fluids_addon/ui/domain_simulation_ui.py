...
def draw_bake_operator_UI_element(context, ui_box):
    ...
    if not bakeprops.is_simulation_running:
        if bakeprops.is_autosave_available:
            ...
            operator_text = _z("Resume Baking  (from frame {})").format(frame_str)
        else:
            operator_text = _z("Bake")
        ...
        if stage == 'STATIC':
            operator_text = _z("Exporting static data... ") + pct_string
        elif stage == 'KEYFRAMED':
            operator_text = _z("Exporting keyframe data... ") + pct_string
        elif stage == 'ANIMATED':
            operator_text = _z("Exporting animated data... ") + pct_string
        else:
            operator_text = _z("Exporting data... ")

    elif not bakeprops.is_bake_initialized:
        operator_text = _z("Baking in progress... initializing")
    elif bakeprops.is_bake_cancelled:
        if bakeprops.is_safe_to_exit:
            safety_str = _z("Safe to quit Blender")
        else:
            safety_str = _z("Do NOT quit Blender")
        operator_text = _z("Cancelling... ") + safety_str
    else:
        ...
        operator_text = _z("Baking in progress...   ") + frames_string
    ...
    if not bakeprops.is_simulation_running and bakeprops.is_autosave_available:
        ...
        if simprops.enable_savestates:
            if simprops.get_num_savestate_enums() < 100:
                ...
                row.prop(simprops, "selected_savestate_int", text=_z("Resume from frame"))
    ...
    if dprops.bake.is_simulation_running:
        ...
        if dprops.stats.is_estimated_time_remaining_available:
            row.label(text=_z("Estimated Time Remaining:    ") + dprops.stats.get_time_remaining_string(context))
        else:
            row.label(text=_z("Calculating time remaining..."))


def draw_bake_operator(self, context, box):
    box.label(text=_z("Bake Simulation:"))
...
def draw_more_bake_settings(self, context, box):
    ...
    row.label(text=_z("More Bake Settings"))
    ...
    subbox = box.box()
    column = subbox.column(align=True)
    column.label(text=_z("Settings and Mesh Export:"))
    ...
    row.label(text=_z("Skip Mesh Re-Export:"))
    ...
    if sprops.skip_mesh_reexport_expanded:
        ...
        if num_flip_objects > flip_object_count_limit:
            ...
            column.alert = True
            column.label(text=_z("Menu Unavailable"), icon="ERROR")
            column.label(text=_z("This menu is only available in scenes containing {} FLIP objects or fewer").format(flip_object_count_limit), icon="ERROR")
            column.label(text=_z("Current number of FLIP objects: ") + str(num_flip_objects), icon="ERROR")
        else:
            column = subbox.column()
            column.label(text=_z("Object Motion Type:"))
            row = column.row(align=True)
            ...
            if sprops.mesh_reexport_type_filter == 'MOTION_FILTER_TYPE_ALL':
                ...
                motion_type_string = _z("simulation")
                ...
                motion_type_string = _z("keyframed")
                ...
                motion_type_string = _z("animated")

            if len(filtered_objects) == 0:
                column.label(text=_z("{} No {} objects found...").format(indent_str, motion_type_string))
            else:
                split = column.split()
                column_left = split.column(align=True)
                ...
                column_left.label(text=_z("Object"))
                op_box = column_left.box()
                op_box.label(text="")

                if is_all_filter_selected:
                    column_animated.label(text="")
                    column_animated.label(text=_z("Export Animated"))
                    ...
                    row.label(text=_z("All"))

                column_middle.label(text="")
                column_middle.label(text=_z("Skip Re-Export"))
                ...
                column_right.label(text=_z("Force Export"))
                column_right.label(text=_z("On Next Bake"))
                ...
                for ob in filtered_objects:
                    ...
                    if is_all_filter_selected:
                        column_animated.prop(pgroup, "export_animated_mesh", text=_z("animated"), toggle=True)
                    column_middle.prop(pgroup, "skip_reexport", text=_z("skip"), toggle=True)
                    column_right.prop(pgroup, "force_reexport_on_next_bake", text=_z("force"), toggle=True)

    subbox = box.box()
    column = subbox.column(align=True)
    column.label(text=_z("Savestates:"))
    ...
    row = column.row()
    row.alignment = 'RIGHT'
    row.label(text=_z("Generate savestate every"))
    column = split.column()
    split = column.split()
    column = split.column()
    row = column.row(align=True)
    row.prop(sprops, "savestate_interval", text="")
    row.label(text=_z("frames"))
    column = subbox.column(align=True)
    column.enabled = sprops.enable_savestates
...
def draw_resolution_settings(self, context, master_column):
    ...
    row.label(text=_z("Grid Resolution:"))

    if not sprops.simulation_resolution_expanded:
        row = column_right.row(align=True)
        row.prop(sprops, "resolution", text=_z("Resolution"))
    else:
        column = box.column(align=True)
        ...
        column_right.prop(sprops, "auto_preview_resolution", text=_z("Use Recommended"))
        ...
        if not dprops.bake.is_simulation_running and sprops.is_current_grid_upscaled():
            ...
            subbox = box.box()
            column = subbox.column(align=True)
            column.label(text=_z("Increased resolution detected"))
            row = column.row()
            row.prop(sprops, "upscale_resolution_tooltip", icon="QUESTION", emboss=False, text="")
            row.label(text=_z("Simulation will be upscaled on resume."))
            column.label(text=indent*" " + _z("     Cache resolution:    ") + str(old_resolution))
            column.label(text=indent*" " + _z("     Current resolution:  ") + str(sprops.resolution))
    ...
    row.label(text=_z("Grid Info:"))
    row = row.row()
    row.alignment = 'RIGHT'
    row.prop(dprops.debug, "grid_display_mode", text="")
    row.prop(dprops.debug, "display_simulation_grid", text=_z("Visualize Grid"))
    ...
    if sprops.grid_info_expanded:
        ...
        row.alignment = 'RIGHT'
        row.label(text=_z("Voxels 3D ="))

        row = column_middle.row(align=True)
        row.alignment = 'RIGHT'
        row.label(text=_z("Dimensions 3D ="))

        row = column_middle.row(align=True)
        row.alignment = 'RIGHT'
        row.label(text=_z("Voxel Size ="))

        row = column_middle.row(align=True)
        row.alignment = 'RIGHT'
        row.label(text=_z("Voxel Count ="))
        ...
        if show_documentation:
            ...
            column.operator(
                    "wm.url_open", 
                    text=_z("What is the simulation grid?"), 
                    icon="WORLD"
    ...
    row.label(text=_z("Simulation Method:"))
    ...
    if not sprops.world_scale_expanded:
        ...
        if wprops.world_scale_mode == 'WORLD_SCALE_MODE_RELATIVE':
            row = column_left.row(align=True)
            row.alignment = 'RIGHT'
            row.label(text=_z("1 Blender Unit = "))
            column_right.prop(wprops, "world_scale_relative")
        else:
            row = column_left.row(align=True)
            row.alignment = 'RIGHT'
            row.label(text=_z("Domain Length = "))
        ...
        if show_documentation:
            ...
            column.operator(
                    "wm.url_open", 
                    text=_z("World Scaling Documentation"), 
                    icon="WORLD"
    ...
    row.label(text=_z("Domain Boundary Collisions:"))
...
def draw_time_settings(self, context, box):
    ...
    row.label(text=_z("Frame Rate and Time Scale:"))
    ...
    column = box.column(align=True)
    column.label(text=_z("Frame Rate:"))
    ...
    if sprops.frame_rate_mode == 'FRAME_RATE_MODE_SCENE':
        column.prop(context.scene.render, "fps", text=_z("Scene Frame Rate"))
    else:
        column.prop(sprops, "frame_rate_custom", text=_z("Custom Frame Rate"))

    column = box.column(align=True)
    column.label(text=_z("Time Scale:"))
    ...
    if sprops.time_scale_mode == 'TIME_SCALE_MODE_CUSTOM':
        column.prop(sprops, "time_scale", text=_z("Custom Time Scale"))
    elif sprops.time_scale_mode == 'TIME_SCALE_MODE_RIGID_BODY':
        if bpy.context.scene.rigidbody_world is not None:
            column.prop(bpy.context.scene.rigidbody_world, "time_scale", text=_z("Rigid Body Time Scale"))
        else:
            row = column.row(align=True)
            row.label(text=_z("No Rigid Body World: "))
        ...
        column_left = split.column(align=True)
        column_right = split.column(align=True)
        column_left.label(text=_z("Soft Body Object:"))
        ...
        if soft_body_modifier is not None:
            row.prop(soft_body_modifier.settings, "speed", text=_z("Soft Body Time Scale"))
        else:
            if sprops.time_scale_object_soft_body is not None:
                row.label(text=_z("No soft body simulation found on object"))
            else:
                row.label(text=_z("No soft body object selected"))

    elif sprops.time_scale_mode == 'TIME_SCALE_MODE_CLOTH':
        split = column.split(align=True)
        column_left = split.column(align=True)
        column_right = split.column(align=True)
        column_left.label(text=_z("Cloth Object:"))
        ...
        if cloth_modifier is not None:
            row.prop(cloth_modifier.settings, "time_scale", text=_z("Cloth Time Scale"))
        else:
            if sprops.time_scale_object_cloth is not None:
                row.label(text=_z("No cloth simulation found on object"))
            else:
                row.label(text=_z("No cloth object selected"))
        ...
        if vcu.is_blender_282():
            ...
            column_left = split.column(align=True)
            column_right = split.column(align=True)
            column_left.label(text=_z("Fluid Domain Object:"))
            ...
            if fluid_modifier is not None:
                row.prop(fluid_modifier.domain_settings, "time_scale", text=_z("Fluid Time Scale"))
            else:
                if sprops.time_scale_object_fluid is not None:
                    row.label(text=_z("No Mantaflow domain found on object"))
                else:
                    row.label(text=_z("No Mantaflow domain object selected"))
        else:
            column.label(text=_z("Mantaflow fluid simulation time scale mode"))
            column.label(text=_z("only supported in Blender 2.82 or later"))
...
class FLIPFLUID_PT_DomainTypePanel(bpy.types.Panel):
    ...
    bl_label = _z("FLIP Fluid Simulation")
    bl_description = "FLIP Fluid Simulation"
