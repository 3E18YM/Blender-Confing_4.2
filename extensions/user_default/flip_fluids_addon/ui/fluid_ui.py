...
class FLIPFLUID_PT_FluidTypePanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        column.prop(fluid_props, "priority", text=_z("Priority Level"))

        box = self.layout.box()
        box.label(text=_z("Fluid Velocity Mode:"))
        row = box.row(align=True)
        row.prop(fluid_props, "fluid_velocity_mode", expand=True)

        if fluid_props.fluid_velocity_mode == 'FLUID_VELOCITY_MANUAL':
            column = box.column(align=True)
            column.label(text=_z("Fluid Velocity:"))
            row = column.row(align=True)
            ...
            column_left.label(text=_z("Fluid Speed:"))
        ...
        if vcu.get_addon_preferences().is_developer_tools_enabled():
            ...
            if vcu.is_blender_293():
                ...
                if dprops is not None and not show_lifetime:
                    ...
                    row.prop(fluid_props, "lifetime_variance", text=_z("Variance"))
                if dprops is None:
                    row.label(text=_z("Domain required for this option"))
                column.separator()
