...
class FLIPFLUID_PT_DomainTypeCachePanel(bpy.types.Panel):
    ...
    bl_label = _z("FLIP Fluid Cache")
    bl_description = "FLIP Fluid Cache"
    ...
    def draw(self, context):
        ...
        #row.alignment = 'LEFT'
        row.label(text=_z("Cache Directory:"))
        ...
        row.label(text=_z("Link existing exported geometry:"))
        ...
        row.label(text=_z("Cache Operators:"))

        if not cprops.cache_operators_expanded:
            if dprops.stats.is_cache_info_available:
                free_text = _z("Free ({})").format(format_bytes(self, dprops.stats.cache_bytes.get()))
            else:
                free_text = _z("Free")
        ...
        if cprops.cache_operators_expanded:
            ...
            column_right.prop(cprops, "clear_cache_directory_logs", text=_z("Free log files"))
            column_right.prop(cprops, "clear_cache_directory_export", text=_z("Free export files"))
