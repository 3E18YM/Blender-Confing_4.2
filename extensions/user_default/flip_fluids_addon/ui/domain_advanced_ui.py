...
class FLIPFLUID_PT_DomainTypeAdvancedPanel(bpy.types.Panel):
    ...
    bl_label = _z("FLIP Fluid Advanced Settings")
    bl_description = "FLIP Fluid Advanced Settings"
    ...
    def draw(self, context):
        ...
        if show_documentation:
            ...
            column.operator(
                "wm.url_open", 
                text=_z("Advanced Settings Documentation"), 
                icon="WORLD"
        ...
        row.label(text=_z("Frame Substeps:"))

        if not aprops.frame_substeps_expanded:
            row = column_right.row(align=True)
            row.prop(aprops.min_max_time_steps_per_frame, "value_min", text=_z("Min"))
            row.prop(aprops.min_max_time_steps_per_frame, "value_max", text=_z("Max"))
        ...
        if aprops.frame_substeps_expanded:
            ...
            if wprops.enable_surface_tension and aprops.min_max_time_steps_per_frame.value_max < wprops.minimum_surface_tension_substeps:
                ...
                row.label(text=_z("  Warning: Not Enough Max Substeps"))
        ...
        if aprops.simulation_method_expanded:
            ...
            if show_documentation:
                ...
                column.operator(
                    "wm.url_open", 
                    text=_z("What are applications of the PIC/FLIP Ratio?"), 
                    icon="WORLD"
        ...
        row.label(text=_z("Simulation and Particle Stability:"))
        ...
        row.label(text=_z("Multithreading and Performance:"))

        if not aprops.multithreading_expanded:
            info_text = ""
            if aprops.threading_mode == 'THREADING_MODE_AUTO_DETECT':
                info_text = _z("Auto-detect {} threads").format(aprops.num_threads_auto_detect)
            elif aprops.threading_mode == 'THREADING_MODE_FIXED':
                info_text = _z("Fixed {} threads").format(aprops.num_threads_fixed)
        ...
        if aprops.multithreading_expanded:
            ...
            if show_documentation:
                ...
                column.operator(
                    "wm.url_open", 
                    text=_z("CPU usage is under 100%, is this normal?"), 
                    icon="WORLD"
        ...
        row.label(text=_z("Warnings and Errors:"))
