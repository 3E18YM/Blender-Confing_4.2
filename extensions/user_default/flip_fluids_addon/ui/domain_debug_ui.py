...
class FLIPFLUID_PT_DomainTypeDebugPanel(bpy.types.Panel):
    ...
    bl_label = _z("FLIP Fluid Debug")
    bl_description = "FLIP Fluid Debug"
    ...
    def draw(self, context):
        ...
        if show_documentation:
            ...
            column.operator(
                "wm.url_open", 
                text=_z("Debug Documentation"), 
                icon="WORLD"
            ).url = "https://github.com/rlguy/Blender-FLIP-Fluids/wiki/Domain-Debug-Settings"
            column.operator(
                "wm.url_open", 
                text=_z("How to use grid visualization and obstacle debugging"), 
                icon="WORLD"
        ...
        row.label(text=_z("Grid Visualization:"))

        if gprops.grid_display_settings_expanded:
            split = vcu.ui_split(box, align=True, factor=0.3)
            column = split.column(align=True)
            column.prop(gprops, "display_simulation_grid", text=_z("Display Grid"))
            ...
            column = split.column(align=True)
            column.prop(gprops, "grid_display_scale", text=_z("Draw Scale"))

            split = vcu.ui_split(box, align=True, factor=0.3)
            column = split.column(align=True)
            column.enabled = gprops.display_simulation_grid
            column.label(text=_z("Enabled Grids:"))
            column.label(text=_z("Grid Colors:"))
            column.label(text=_z("Grid Offsets:"))
            column = split.column(align=True)
            column.enabled = gprops.display_simulation_grid
        ...
        row.label(text=_z("Fluid Particle Debugging:"))
        ...
        if gprops.particle_debug_settings_expanded:
            ...
            column = split.column()
            column.label(text=_z("Particle Size:"))
            column.label(text=_z("Draw Bounds:"))
            column = split.column()
        ...
        row.label(text=_z("Force Field Debugging:"))
        ...
        if gprops.force_field_debug_settings_expanded:
            ...
            column.label(text=_z("Force Field Display Settings:"))
            ...
            split = vcu.ui_split(column, factor=0.33)
            column = split.column()
            column.label(text=_z("Display Amount:"))
            column.label(text=_z("Line Size:"))
            column = split.column()
