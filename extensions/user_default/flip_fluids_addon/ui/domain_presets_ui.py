...
class FLIPFLUID_PT_DomainTypePresetsPanel(bpy.types.Panel):
    ...
    bl_label = _z("FLIP Fluid Presets")
    bl_description = "FLIP Fluid Presets"
    ...
    def draw_preset_selector(self, context):
        ...
        column.label(text=_z("Preset Package:"))
        ...
        if current_package_info["use_custom_icons"]:
            ...
            row_left.prop(preprops, "preview_preset", text=_z("Auto Assign Preset"))

            if preprops.current_preset != "PRESET_NONE":
                is_on_stack = preprops.preset_stack.is_preset_in_stack(preprops.current_preset)
                op_text = _z("Added to Stack") if is_on_stack else _z("Add to Stack")
    ...
    def draw_preset_stack(self, context):
        ...
        for pidx,p in enumerate(preprops.preset_stack.preset_stack):
            ...
            if "icon" in info and info['icon'] in preset_icons:
                row_left.label(text=_z(info['name']), icon_value=preset_icons.get(info['icon']).icon_id)
            else:
                row_left.label(text=" "*5 + _z(info['name']))
    ...
    def draw_preset_manager(self, context):
        ...
        row.label(text=_z("Preset Manager"))
        if preprops.preset_manager_expanded:
            column = box.column(align=True)
            column.label(text=_z("Package Operators:"))
            ...
            column = box.column(align=True)
            column.label(text=_z("Preset Operators:"))
    ...
    def draw_default_settings_operators(self, context):
        ...
        box = self.layout.box()
        column = box.column(align=True)
        column.label(text=_z("Default Settings:"))
    ...
    def draw(self, context):
        if not preset_library.get_user_package_info_list():
            self.layout.label(text=_z("This feature is missing data and will be disabled."))
            self.layout.label(text=_z("Please contact the developers if you think this is an error."))
            return
