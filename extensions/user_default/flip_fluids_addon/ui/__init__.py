...
def append_to_PHYSICS_PT_add_panel(self, context):
    ...
    if obj.flip_fluid.is_active:
        ...
        if installation_utils.is_experimental_build() and addon_prefs.enable_experimental_build_warning:
            ...
            column.label(text=_z("This is an experimental build of the FLIP Fluids addon"), icon='ERROR')
            column.label(text=_z("Not for production. Use at your own risk."), icon='ERROR')
            column.label(text=_z("Please read before using:"), icon='ERROR')
            column.operator(
                    "wm.url_open", 
    ...
    if len(flip_fluids_installations) > 1:
        box = self.layout.box()
        box.label(text=_z("Installation Error Detected"), icon="ERROR")
        box.label(text=_z("Multiple version of the FLIP Fluids addon enabled:"), icon="ERROR")
        for install in flip_fluids_installations:
            box.label(text=" "*10 + install['addon_name'] + " (" + install['module_name'] + ")")
        box.label(text=_z("Only 1 version of the add-on can be enabled"), icon="ERROR")
        box.label(text=_z("Disable all other versions in the Blender addon preferences"), icon="ERROR")
        box.label(text=_z("Restart Blender"), icon="ERROR")
