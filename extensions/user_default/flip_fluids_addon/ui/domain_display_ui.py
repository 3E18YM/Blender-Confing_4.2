...
def draw_simulation_display_settings(self, context):
    ...
    if not scene_props.show_viewport or not scene_props.show_render:
        ...
        if not scene_props.show_viewport and not scene_props.show_render:
            visibility_text += _z("Disabled in Viewport + Render")
        elif not scene_props.show_viewport:
            visibility_text += _z("Disabled in Viewport")
        elif not scene_props.show_render:
            visibility_text += _z("Disabled in Render")
    ...
    if scene_props.show_viewport and scene_props.show_render:
        row = row.row(align=True)
        row.alignment = 'RIGHT'
        row.label(text=_z("Visibility Enabled"), icon="CHECKMARK")
    ...
    column_left.prop(scene_props, "show_render", text=_z("Show In Render"), icon="RESTRICT_RENDER_OFF")

    column_right = split.column()
    column_right.prop(scene_props, "show_viewport", text=_z("Show In Viewport"), icon="RESTRICT_VIEW_OFF")
...
def draw_surface_display_settings(self, context, menu_expand_prop_group=None):
    ...
    column_left = split.column()
    column_left.label(text=_z("Render Display Mode:"))
    column_left.prop(rprops, "render_display", expand=True)

    column_right = split.column()
    column_right.label(text=_z("Viewport Display Mode:"))
    column_right.prop(rprops, "viewport_display", expand=True)

    column_left.label(text=_z("Surface Material:"))
...
def draw_fluid_particle_display_settings(self, context, menu_expand_prop_group=None):
    ...
    if is_fluid_particles_enabled:
        ...
        row.label(text=_z("Fluid Particle Display and Render:"))
    else:
        split = column.split()
        ...
        c.alignment = 'RIGHT'
        c.enabled = False
        c.label(text=_z("Enable in 'Particles' panel"))
    ...
    if getattr(menu_expand_prop_group, "fluid_particle_display_settings_expanded"):
        ...
        column_left.label(text=_z("Final Display Amount:"))
        ...
        column_right.label(text=_z("Preview Display Amount:"))
        ...
        column = subbox.column(align=True)
        column.label(text=_z("Particle Display Settings:"))
        column.separator()

        bl_mod = get_motion_blur_geometry_node_modifier(bl_fluid_particles_mesh_cache)
        row = column.row(align=True)
        row.alignment = 'LEFT'
        row.label(text=_z("Fluid Particles:"))
        if is_fluid_particles_enabled:
            draw_fluid_particles_motion_blur_geometry_node_properties(row, bl_mod)
        else:
            row.label(text=_z("Enable Fluid Particle feature to view full particle settings"), icon='INFO')
        ...
        column_left = split.column()
        column_right = split.column()
        column_left.label(text=_z("Fluid Particle Material:"))
...
def draw_whitewater_particles_motion_blur_geometry_node_properties(ui_row, bl_mod):
    if bl_mod is None:
        ...
        ui_row.operator(
            ...
            text=_z("Initialize Geometry Nodes - Missing FF_MotionBlurWhitewater modifier"),
            icon="ERROR"
    ...
    if "Input_6" in bl_mod:
        ui_row.prop(bl_mod, '["Input_6"]',  text=_z("Scale"))
    if "Input_4" in bl_mod:
        ui_row.prop(bl_mod, '["Input_4"]',  text=_z("Blur Scale"))
    if "Input_8" in bl_mod:
        ui_row.prop(bl_mod, '["Input_8"]',  text=_z("Motion Blur"))
    if "Input_9" in bl_mod:
        ui_row.prop(bl_mod, '["Input_9"]',  text=_z("Point Cloud"))
...
def draw_fluid_particles_motion_blur_geometry_node_properties(ui_row, bl_mod):
    if bl_mod is None:
        ...
        ui_row.operator(
            "flip_fluid_operators.helper_initialize_cache_objects", 
            text=_z("Initialize Geometry Nodes - Missing FF_MotionBlurFluidParticles modifier"),
            icon="ERROR"
...
def draw_whitewater_display_settings(self, context, menu_expand_prop_group=None):
    ...
    if is_whitewater_enabled:
        ...
        row.label(text=_z("Whitewater Display and Render:"))
    else:
        split = column.split()
        left_column = split.column()
        row = left_column.row(align=True)
        ...
        c.alignment = 'RIGHT'
        c.enabled = False
        c.label(text=_z("Enable in 'Whitewater' panel"))
    ...
    if getattr(menu_expand_prop_group, "whitewater_display_settings_expanded"):
        ...
        column.label(text=_z("Render Display Mode:"))
        column.prop(rprops, "whitewater_render_display", expand=True)

        column = split.column(align=True)
        column.label(text=_z("Viewport Display Mode:"))
        ...
        split = column.split()
        column = split.column(align=True)
        column.label(text=_z("Final Display Amount:"))
        ...
        column = split.column(align=True)
        column.label(text=_z("Preview Display Amount:"))
        ...
        for idx, bl_object in enumerate(mesh_cache_objects):
            ...
            row.label(text=_z(whitewater_labels[idx]))
            row = column2.row(align=True)
            if is_whitewater_enabled:
                draw_whitewater_particles_motion_blur_geometry_node_properties(row, bl_mod)
            else:
                row.label(text=_z("Enable Whitewater feature to view full particle settings"), icon='INFO')
        ...
        column = box.column(align=True)
        column.label(text=_z("Particle Materials:"))
        column.prop(mprops, "whitewater_foam_material", text=_z("Foam"))
        column.prop(mprops, "whitewater_bubble_material", text=_z("Bubble"))
        column.prop(mprops, "whitewater_spray_material", text=_z("Spray"))
        column.prop(mprops, "whitewater_dust_material", text=_z("Dust"))
...
class FLIPFLUID_PT_DomainTypeDisplayPanel(bpy.types.Panel):
    ...
    bl_label = _z("FLIP Fluid Display and Render Settings")
    bl_description = "FLIP Fluid Display and Render Settings"
    ...
    def draw(self, context):
        ...
        if show_documentation:
            ...
            column.operator(
                "wm.url_open", 
                text=_z("Display and Render Settings Documentation"), 
                icon="WORLD"
            ).url = "https://github.com/rlguy/Blender-FLIP-Fluids/wiki/Domain-Display-Settings"
            column.operator(
                "wm.url_open", 
                text=_z("Rendering from the command line"), 
                icon="WORLD"
            ...
            column.operator(
                "wm.url_open", 
                text=_z("Simulation meshes not appearing in viewport/render"), 
                icon="WORLD"
