...
class FLIPFLUID_PT_DomainTypeFluidWorldPanel(bpy.types.Panel):
    ...
    bl_label = _z("FLIP Fluid World")
    bl_description = "FLIP Fluid World"
    ...
    def draw(self, context):
        ...
        if show_documentation:
            ...
            column.operator(
                "wm.url_open", 
                text=_z("World Panel Documentation"), 
                icon="WORLD"
        ...
        if wprops.world_scale_settings_expanded:
            ...
            row = column_left.row(align=True)
            row.alignment = 'RIGHT'
            row.label(text=_z("Simulation dimensions:  X = "))
            row = column_left.row(align=True)
            row.alignment = 'RIGHT'
            row.label(text="Y = ")
            row = column_left.row(align=True)
        ...
        if show_documentation:
            ...
            column.operator(
                "wm.url_open", 
                text=_z("How to use relative and absolute scaling modes"), 
                icon="WORLD"
        ...
        row.label(text=_z("Gravity and Force Fields:"))

        if wprops.force_field_settings_expanded:

            subbox = box.box()
            subbox.label(text=_z("Gravity:"))
            ...
            row = column_left.row(align=True)
            row.alignment = 'RIGHT'
            row.label(text=_z("magnitude = ") + mag_str)
            row = column_left.row(align=True)
            row.alignment = 'RIGHT'
            row.label(text=_z("g-force = ") + gforce_str)

            column_right.enabled = not (wprops.gravity_type == 'GRAVITY_TYPE_SCENE')

            if wprops.gravity_type == 'GRAVITY_TYPE_SCENE':
                column_right.prop(context.scene, "use_gravity", text=_z("Gravity Enabled"))
            ...
            subbox = box.box()
            subbox.label(text=_z("Force Field Resolution:"))
            column = subbox.column(align=True)
            ...
            subbox = box.box()
            subbox.label(text=_z("Force Field Weights:"))
            column = subbox.column(align=True)
        ...
        if wprops.viscosity_settings_expanded:
            ...
            if vcu.get_addon_preferences().is_developer_tools_enabled():
                row = row.row(align=True)
                row.enabled = wprops.enable_viscosity
                row.prop(attrprops, "enable_viscosity_attribute", text=_z("Variable Viscosity"))

            column = box.column(align=True)
            column.enabled = wprops.enable_viscosity

            if is_variable_viscosity_enabled:
                column.label(text=_z("Variable viscosity values can be set in the"), icon='INFO')
                column.label(text=_z("Fluid or Inflow physics properties menu"), icon='INFO')
            else:
                column.prop(wprops, "viscosity", text=_z("Base"))
                column.prop(wprops, "viscosity_exponent", text=_z("Exponent"))

            column.prop(wprops, "viscosity_solver_error_tolerance", text=_z("Solver Accuracy"), slider=True)

            if is_variable_viscosity_enabled:
                column.label(text="")
            else:
                total_viscosity = wprops.viscosity * (10**(-wprops.viscosity_exponent))
                total_viscosity_str = _z("Total viscosity   =   {}").format(format_number_precision(self, total_viscosity))
        ...
        if show_documentation:
            ...
            column.operator(
                "wm.url_open", 
                text=_z("Viscosity Documentation"), 
                icon="WORLD"
        ...
        row.label(text=_z("Surface Tension:"))
        ...
        if wprops.surface_tension_settings_expanded:
            ...
            column_right.prop(wprops, "surface_tension", text=_z("Base"))
            column_right.prop(wprops, "surface_tension_exponent", text=_z("Exponent"))
            column_right.prop(wprops, "surface_tension_accuracy", text=_z("Solver Accuracy"))
            ...
            if wprops.enable_surface_tension and wprops.minimum_surface_tension_substeps > aprops.min_max_time_steps_per_frame.value_max:
                ...
                row.label(text=_z("  Warning: Too Many Substeps"))

        if show_documentation:
            column = box.column(align=True)
            column.operator(
                "wm.url_open", 
                text=_z("Surface Tension Documentation"), 
                icon="WORLD"
        ...
        row.label(text=_z("Sheeting Effects:"))

        if wprops.sheeting_settings_expanded:
            box.label(text=_z("Sheeting Effects:"))
            column = box.column(align=True)
            split = column.split(align=True)
            column_left = split.column(align=True)
            ...
            indent_str = 5 * " "
            column.label(text=_z("Obstacle Sheeting:"))
            if len(obstacle_objects) == 0:
                ...
                for ob in obstacle_objects:
                    ...
                    column_right.prop(pgroup, "sheeting_strength", text=_z("Strength Scale"))

        if show_documentation:
            column = box.column(align=True)
            column.operator(
                "wm.url_open", 
                text=_z("Sheeting Effects Documentation"), 
                icon="WORLD"
        ...
        row.label(text=_z("Friction:"))

        if not wprops.friction_settings_expanded:
            row = row.row(align=True)
            row.alignment = 'RIGHT'
            row.label(text=_z("Boundary Friction  "))
        ...
        if wprops.friction_settings_expanded:
            ...
            split = column.split(align=True)
            column_left = split.column()
            column_left.label(text=_z("Boundary Friction:"))
            column_right = split.column()
            ...
            row.label(text=_z("Obstacle Friction:"))
