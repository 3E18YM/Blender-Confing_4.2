...
class FLIPFLUID_PT_FLIPFluidsAddonDisabledPanel(bpy.types.Panel):
    ...
    bl_label = _z("FLIP Fluids Addon Disabled")
    bl_description = "FLIP Fluids Addon Disabled"
    ...
    def draw(self, context):
        ...
        row.alert = True
        row.label(text=_z("FLIP Fluids Addon has been disabled in this Blend file"), icon="INFO")
        row = column.row(align=True)
        row.alert = True
        row.label(text=_z("Click to re-enable and use the addon"), icon="INFO")
