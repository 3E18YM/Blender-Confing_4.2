...
class FLIPFLUID_PT_InflowTypePanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if show_documentation:
            ...
            column.operator(
                "wm.url_open", 
                text=_z("Inflow objects must have manifold/watertight geometry"), 
                icon="WORLD"
        ...
        column.prop(inflow_props, "priority", text=_z("Priority Level"))

        column.separator()
        box = self.layout.box()
        box.label(text=_z("Inflow Velocity Mode:"))
        row = box.row(align=True)
        row.prop(inflow_props, "inflow_velocity_mode", expand=True)

        if inflow_props.inflow_velocity_mode == 'INFLOW_VELOCITY_MANUAL':
            column = box.column(align=True)
            column.label(text=_z("Inflow Velocity:"))
            row = column.row(align=True)
            ...
            column_left.label(text=_z("Inflow Speed:"))
            column_left.prop(inflow_props, "inflow_speed")
                
            column_right = split.column(align=True)
            column_right.label(text=_z("Local Axis:"))
            ...
            column_right = split.column(align=True)
            column_right.label(text=_z("Target Object:"))
        ...
        if vcu.get_addon_preferences().is_developer_tools_enabled():
            box = self.layout.box()
            box.label(text=_z("Geometry Attributes:"))
            column = box.column(align=True)
            if vcu.is_blender_293():
                ...
                if dprops is not None and not show_color:
                    row.operator("flip_fluid_operators.enable_color_attribute_tooltip", 
                                 text=_z("Enable Color Attribute"), icon="PLUS", emboss=False)
                if dprops is None:
                    row.label(text=_z("Domain required for this option"))
                column.separator()
                ...
                if dprops is not None and not show_viscosity:
                    row.operator("flip_fluid_operators.enable_viscosity_attribute_tooltip", 
                                 text=_z("Enable Viscosity Attribute"), icon="PLUS", emboss=False)
                if dprops is None:
                    row.label(text=_z("Domain required for this option"))
                column.separator()
                ...
                if dprops is not None and not show_lifetime:
                    row.operator("flip_fluid_operators.enable_lifetime_attribute_tooltip", 
                                 text=_z("Enable Lifetime Attribute"), icon="PLUS", emboss=False)
                elif dprops is not None:
                    row.alignment = 'EXPAND'
                    row.prop(inflow_props, "lifetime_variance", text=_z("Variance"))
                if dprops is None:
                    row.label(text=_z("Domain required for this option"))
                column.separator()
                ...
                if dprops is not None and not show_source_id:
                    row.operator("flip_fluid_operators.enable_source_id_attribute_tooltip", 
                                 text=_z("Enable Source ID Attribute"), icon="PLUS", emboss=False)
                if dprops is None:
                    row.label(text=_z("Domain required for this option"))
                column.separator()
            else:
                column.enabled = False
                column.label(text=_z("Geometry attribute features are only available in"), icon='ERROR')
                column.label(text=_z("Blender 2.93 or later"), icon='ERROR')
