...
def __get_domain_data_dict(context, dobj):
    ...
    if not 'advanced' in d:
        errmsg = _z("This error may indicate that either (1) A Blender restart is required ")
        errmsg += _z("to complete installation of the FLIP Fluids addon. Save, restart Blender, and ")
        errmsg += _z("try again. Or (2) This version of the FLIP Fluids addon is not compatible ")
        errmsg += _z("with the Blender version. Update to the latest version of the FLIP Fluids ")
        errmsg += _z("addon and try again. Contact the developers at support@flipfluids.com for assistance.")
        
        bpy.ops.flip_fluid_operators.display_error(
                'INVOKE_DEFAULT',
                error_message=_z("Installation or Compatibility Error"),
