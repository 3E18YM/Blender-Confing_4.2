...
class DomainSimulationProperties(bpy.types.PropertyGroup):
    ...
    frame_range_mode = EnumProperty(
            ...
            description="Frame Range Mode\nFrame range to use for baking the simulation",
    ...
    update_settings_on_resume = BoolProperty(
            name=_z("Update Settings on Resume"),
            description="Update Settings on Resume\nUpdate simulation settings and meshes when resuming a bake."
    ...
    mesh_reexport_type_filter = EnumProperty(
            name=_z("Object Motion Type"),
            description="Object Motion Type\nFilter objects by motion type for skip re-export list display",
    ...
    enable_savestates = BoolProperty(
            name=_z("Enable Savestates"),
            description="Enable Savestates\nGenerate savestates/checkpoints as the simulation progresses."
    ...
    savestate_interval = IntProperty(
            name=_z("Savestate Interval"),
            description="Savestate Interval\nNumber of frames between each savestate",
            min=1,
            default=50,
            options={'HIDDEN'},
            ); exec(conv("savestate_interval"))
    delete_outdated_savestates = BoolProperty(
            name=_z("Delete Outdated Savestates on Resume"),
            description="Delete Outdated Savestates on Resume\nWhen resuming a simulation from a previous frame, delete"
                " all savestates that are ahead in the timeline",
            default=True,
            options = {'HIDDEN'},
            ); exec(conv("delete_outdated_savestates"))
    delete_outdated_meshes = BoolProperty(
            name=_z("Delete Outdated Meshes on Resume"),
            description="Delete Outdated Meshes on Resume\nWhen resuming a simulation from a previous frame, delete"
                " all simulation meshes that are ahead in the timeline",
            default=True,
            options = {'HIDDEN'},
            ); exec(conv("delete_outdated_meshes"))
    selected_savestate = EnumProperty(
            name=_z("Selected Savestate"),
            description="Selected Savestate\nResume simulation from this savestate frame",
    ...
    resolution = IntProperty(
            name=_z("Resolution"),
            description="Resolution\nDomain grid resolution. This value specifies the number of"
    ...
    preview_resolution = IntProperty(
            name=_z("Preview Resolution"),
            description="Preview Resolution\nThe resolution to use for generating lower quality meshes for"
    ...
    auto_preview_resolution = BoolProperty(
            name=_z("Recommended"),
            description="Recommended\nSet recommended preview resolution based on domain resolution and"
    ...
    lock_cell_size = BoolProperty(
            name=_z("Lock Voxel Size"),
            description="Lock Voxel Size\nLock the current voxel size and update the grid"
    ...
    fluid_open_boundary_width = IntProperty(
            name=_z("Open Boundary Width"),
            description="Open Boundary Width\nThe distance (in number of voxels) from the domain boundary that fluid will be"
    ...
    frame_rate_mode = EnumProperty(
            name=_z("Frame Rate Mode"),
            description="Frame Rate Mode\nSelect the frame rate for the simulation animation",
    ...
    time_scale_mode = EnumProperty(
            name=_z("Time Scale Mode"),
            description="Time Scale Mode\nSelect the time scale mode for the simulation. Use either a custom"
    ...
    time_scale_object_soft_body = PointerProperty(
            name=_z("Soft Body Object"), 
            type=bpy.types.Object,
            poll=time_scale_object_soft_body_poll,
            options={'HIDDEN'},
            description = "Soft Body Object"); exec(conv("time_scale_object_soft_body"))
    ...
    time_scale_object_cloth = PointerProperty(
            name=_z("Cloth Object"), 
            type=bpy.types.Object,
            poll=time_scale_object_cloth_poll,
            options={'HIDDEN'},
            description = "Cloth Object"); exec(conv("time_scale_object_cloth"))
    ...
    time_scale_object_fluid = PointerProperty(
            name=_z("Fluid Domain Object"), 
            type=bpy.types.Object,
            poll=time_scale_object_fluid_poll,
            options={'HIDDEN'},
            description = "Fluid Domain Object"); exec(conv("time_scale_object_fluid"))

    time_scale = FloatProperty(
            name=_z("Custom Time Scale"), 
            description="Custom Time Scale\nScale the simulation speed by this value. If set to less than"
    ...
    upscale_resolution_tooltip = BoolProperty(
            name=_z("Upscale Resolution Tooltip"), 
            description="Upscale Resolution Tooltip\nUpscaling converts a lower resolution savestate to a higher resolution savestate"
    ...
    grid_voxels_tooltip = BoolProperty(
            name=_z("Grid Voxels Tooltip"), 
            description="Grid Voxels Tooltip\nThe domain is a 3D grid of cubes called voxels, or cells. This info shows the"
    ...
    grid_dimensions_tooltip = BoolProperty(
            name=_z("Grid Dimensions Tooltip"), 
            description="Grid Dimensions Tooltip\nDisplays the physical scale of the domain on the X/Y/Z axis in meters."
    ...
    grid_voxel_size_tooltip = BoolProperty(
            name=_z("Voxel Size Tooltip"), 
            description="Voxel Size Tooltip\nDisplays the physical size of a single voxel. You can think of a voxel as"
    ...
    grid_voxel_count_tooltip = BoolProperty(
            name=_z("Voxel Count Tooltip"), 
            description="Voxel Count Tooltip\nDisplays the total number of voxels in the domain. Physics are computed for each"
    ...
    def register_preset_properties(self, registry, path):
        ...
        add(path + ".resolution",              _z("Resolution"),              group_id=0)
        add(path + ".preview_resolution",      _z("Preview Resolution"),      group_id=0)
        add(path + ".auto_preview_resolution", _z("Auto Preview Resolution"), group_id=0)
        add(path + ".lock_cell_size",          _z("Lock Cell Size"),          group_id=0)
        add(path + ".frame_rate_mode",         _z("Frame Rate Mode"),         group_id=0)
        add(path + ".frame_rate_custom",       _z("Frame Rate"),              group_id=0)
        add(path + ".time_scale_mode",          _z("Time Scale Mode"),               group_id=0)
        add(path + ".time_scale",              _z("Time Scale"),              group_id=0)
        add(path + ".fluid_boundary_collisions",  _z("Boundary Collisions"),        group_id=0)
        add(path + ".fluid_open_boundary_width",  _z("Open Boundary Width"),        group_id=0)
        add(path + ".frame_range_mode",           _z("Frame Range Mode"),           group_id=1)
        add(path + ".frame_range_custom",         _z("Frame Range (Custom)"),       group_id=1)
        add(path + ".update_settings_on_resume",  _z("Update Settings on Resume"),  group_id=1)
        add(path + ".enable_savestates",          _z("Enable Savestates"),          group_id=1)
        add(path + ".savestate_interval",         _z("Savestate Interval"),         group_id=1)
        add(path + ".delete_outdated_savestates", _z("Delete Outdated Savestates"), group_id=1)
        add(path + ".delete_outdated_meshes",     _z("Delete Outdated Meshes"),     group_id=1)
