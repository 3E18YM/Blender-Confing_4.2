...
class PresetPropertiesUI(bpy.types.PropertyGroup):
    ...
    def generate_column_partition_chunks(self, collection_ids):
        ...
        if 'simulation' in collection_ids:
            chunks.append({'label': _z("FLIP Fluid Simulation"), 'collection': self.simulation})
        if 'render' in collection_ids:
            chunks.append({'label': _z("FLIP Fluid Display Settings"), 'collection': self.render})
        if 'surface' in collection_ids:
            chunks.append({'label': _z("FLIP Fluid Surface"), 'collection': self.surface})
        if 'whitewater' in collection_ids:
            chunks.append({'label': _z("FLIP Fluid Whitewater"), 'collection': self.whitewater})
        if 'world' in collection_ids:
            chunks.append({'label': _z("FLIP Fluid World"), 'collection': self.world})
        if 'materials' in collection_ids:
            chunks.append({'label': _z("FLIP Fluid Materials"), 'collection': self.materials})
        if 'advanced' in collection_ids:
            chunks.append({'label': _z("FLIP Fluid Advanced"), 'collection': self.advanced})
        if 'debug' in collection_ids:
            chunks.append({'label': _z("FLIP Fluid Debug"), 'collection': self.debug})
        if 'stats' in collection_ids:
            chunks.append({'label': _z("FLIP Fluid Stats"), 'collection': self.stats})
    ...
    def generate_columns3(self, collection_id, property_registry):
        ...
        if collection_id == 'simulation':
            collection, label = self.simulation, _z("FLIP Fluid Simulation")
        if collection_id == 'render':
            collection, label = self.render,     _z("FLIP Fluid Display Settings")
        if collection_id == 'surface':
            collection, label = self.surface,    _z("FLIP Fluid Surface")
        if collection_id == 'whitewater':
            collection, label = self.whitewater, _z("FLIP Fluid Whitewater")
        if collection_id == 'world':
            collection, label = self.world,      _z("FLIP Fluid World")
        if collection_id == 'materials':
            collection, label = self.materials,  _z("FLIP Fluid Materials")
        if collection_id == 'advanced':
            collection, label = self.advanced,   _z("FLIP Fluid Advanced")
        if collection_id == 'debug':
            collection, label = self.debug,      _z("FLIP Fluid Debug")
        if collection_id == 'stats':
            collection, label = self.stats,      _z("FLIP Fluid Stats")
...
class NewPresetSettings(bpy.types.PropertyGroup):
    ...
    package = EnumProperty(
            name=_z("Preset Package"),
            description="Preset Package\nAdd preset to this package",
    ...
    icon = StringProperty(
            name=_z("Icon"),
            description="Icon\nIcons should be 256x256 resolution and in PNG format."
    ...
    export_simulation = BoolProperty(
            name=_z("Simulation"),
            description="Simulation\nExport 'FLIP Fluid Simulation' panel settings",
            default=False,
    ...
    export_display = BoolProperty(
            name=_z("Display Settings"),
            description="Display Settings\nExport 'FLIP Fluid Display Settings' panel settings",
            default=False,
            update=lambda self, context=None: self._check_current_display_panel(context),
            ); exec(conv("export_display"))
    export_surface = BoolProperty(
            name=_z("Surface"),
            description="Surface\nExport 'FLIP Fluid Surface' panel settings",
            default=False,
            update=lambda self, context=None: self._check_current_display_panel(context),
            ); exec(conv("export_surface"))
    export_whitewater = BoolProperty(
            name=_z("Whitewater"),
            description="Whitewater\nExport 'FLIP Fluid Whitewater' panel settings",
            default=False,
            update=lambda self, context=None: self._check_current_display_panel(context),
            ); exec(conv("export_whitewater"))
    export_world = BoolProperty(
            name=_z("World"),
            description="World\nExport 'FLIP Fluid World' panel settings",
            default=False,
            update=lambda self, context=None: self._check_current_display_panel(context),
            ); exec(conv("export_world"))
    export_materials = BoolProperty(
            name=_z("Materials"),
            description="Materials\nExport 'FLIP Fluid Materials' panel settings",
            default=False,
            update=lambda self, context=None: self._check_current_display_panel(context),
            ); exec(conv("export_materials"))
    export_advanced = BoolProperty(
            name=_z("Advanced"),
            description="Advanced\nExport 'FLIP Fluid Advanced' panel settings",
            default=False,
            update=lambda self, context=None: self._check_current_display_panel(context),
            ); exec(conv("export_advanced"))
    export_debug = BoolProperty(
            name=_z("Debug"),
            description="Debug\nExport 'FLIP Fluid Debug' panel settings",
            default=False,
            update=lambda self, context=None: self._check_current_display_panel(context),
            ); exec(conv("export_debug"))
    export_stats = BoolProperty(
            name=_z("Stats"),
            description="Stats\nExport 'FLIP Fluid Stats' panel settings",
            default=False,
            update=lambda self, context=None: self._check_current_display_panel(context),
            ); exec(conv("export_stats"))
    ui_sort = BoolProperty(
            name=_z("Sort Attributes"),
            description="Sort Attributes\nSort attributes by enabled/disabled",
            default=False,
    ); exec(conv("ui_sort"))
    current_display_panel = EnumProperty(
        name=_z("Current Preset Display"),
        description="Current Preset Display\nCurrent preset panel to display",
    ...
    def get_preset_panel_selector_enums(self, context=None):
        enums = []
        if self.export_simulation:
            enums.append(('simulation', _z("Simulation"), "FLIP Fluid Simulation", 1))
        if self.export_display:
            enums.append(('render', _z("Display"), "FLIP Fluid Display Settings", 2))
        if self.export_surface:
            enums.append(('surface', _z("Surface"), "FLIP Fluid Surface", 3))
        if self.export_whitewater:
            enums.append(('whitewater', _z("Whitewater"), "FLIP Fluid Whitewater", 4))
        if self.export_world:
            enums.append(('world', _z("World"), "FLIP Fluid World", 5))
        if self.export_materials:
            enums.append(('materials', _z("Materials"), "FLIP Fluid Materials", 6))
        if self.export_advanced:
            enums.append(('advanced', _z("Advanced"), "FLIP Fluid Advanced", 7))
        if self.export_debug:
            enums.append(('debug', _z("Debug"), "FLIP Fluid Debug", 8))
        if self.export_stats:
            enums.append(('stats', _z("Stats"), "FLIP Fluid Stats", 9))

        if not enums:
            enums.append(('NONE', _z("None"), "No panel to select", 0))
        return enums
    ...
    def _get_display_icon_enum(self, context=None):
        ...
        return [('ICON_NONE', _z("None"), "No icon loaded...")]
...
class ExportPresetPackageSettings(bpy.types.PropertyGroup):
    ...
    create_subdirectories = BoolProperty(
            name=_z("Create Missing Subdirectories"),
            description="Create Missing Subdirectories\nCreate missing subdirectories if export directory does not exist",
            default=True,
...
class ImportPresetPackageSettings(bpy.types.PropertyGroup):
    ...
    selected_preset = EnumProperty(
            name=_z("Preset"),
            description="Preset\nSelect a package preset to view details",
...
class EditPresetSettings(bpy.types.PropertyGroup):
    ...
    description = StringProperty(
            name=_z("Description"),
            description="Description\nNew preset description (optional)",
            default=""
            ); exec(conv("description"))
    icon = StringProperty(
            name=_z("Icon"),
