...
class SolverStressProperties(bpy.types.PropertyGroup):
    ...
    stress_level = FloatProperty(
            name=_z("Stress Level"),
            description="Stress Level\nAmount of stress experienced by the solver. If the stress level exceeds"
...
class DomainStatsProperties(bpy.types.PropertyGroup):
    ...
    cache_info_type = EnumProperty(
            name=_z("Cache Info Display Mode"),
            description="Cache Info Display Mode\nType of cache info to display",
    ...
    lock_info_frame_to_timeline = BoolProperty(
            name=_z("Lock To Timeline"),
            description="Lock To Timeline\nSet frame number to current frame in timeline",
            default=True,
    ...
    csv_region_format = EnumProperty(
            name=_z("Region Format"),
            description="Region Format\nCSV region formatting",
    ...
    def register_preset_properties(self, registry, path):
        ...
        add(path + ".cache_info_type",             _z("Info Type"),              group_id=0)
        add(path + ".lock_info_frame_to_timeline", _z("Lock Frame to Timeline"), group_id=0)
        add(path + ".csv_region_format",           _z("CSV Region Format"),      group_id=0)
