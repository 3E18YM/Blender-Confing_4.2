...
class FlipFluidObjectProperties(bpy.types.PropertyGroup):
    ...
    domain = PointerProperty(
            name=_z("Flip Fluid Domain Properties"),
            description="Flip Fluid Domain Properties",
            type=domain_properties.FlipFluidDomainProperties,
            ); exec(conv("domain"))
    fluid = PointerProperty(
            name=_z("Flip Fluid Fluid Properties"),
            description="Flip Fluid Fluid Properties",
            type=fluid_properties.FlipFluidFluidProperties,
            ); exec(conv("fluid"))
    obstacle = PointerProperty(
            name=_z("Flip Fluid Obstacle Properties"),
            description="Flip Fluid Obstacle Properties",
            type=obstacle_properties.FlipFluidObstacleProperties,
            ); exec(conv("obstacle"))
    inflow = PointerProperty(
            name=_z("Flip Fluid Inflow Properties"),
            description="Flip Fluid Inflow Properties",
            type=inflow_properties.FlipFluidInflowProperties,
            ); exec(conv("inflow"))
    outflow = PointerProperty(
            name=_z("Flip Fluid Outflow Properties"),
            description="Flip Fluid Outflow Properties",
            type=outflow_properties.FlipFluidOutflowProperties,
            ); exec(conv("outflow"))
    force_field = PointerProperty(
            name=_z("Flip Fluid Force Field Properties"),
            description="Flip Fluid Force Field Properties",
            type=force_field_properties.FlipFluidForceFieldProperties,
            ); exec(conv("force_field"))
    object_type = EnumProperty(
            name=_z("Type"),
            description="Type\nType of participation in the FLIP fluid simulation",
    ...
    saved_view_settings = PointerProperty(
            name=_z("Saved View Settings"),
            description="Saved View Settings",
    ...
    def register(cls):
        bpy.types.Object.flip_fluid = PointerProperty(
                name=_z("Flip Fluid Object Properties"),
                description="Flip Fluid Object Properties",
                type=cls,
                )
    ...
    def _update_object_type(self, context):
        ...
        if self.object_type == 'TYPE_DOMAIN':
            if bpy.context.scene.flip_fluid.get_num_domain_objects() > 1:
                ...
                errmsg = _z("Only 1 Domain object is supported per Blend file.")
        ...
        if (obj.type == 'EMPTY' or obj.type == 'CURVE') and self.object_type != 'TYPE_FORCE_FIELD' and self.object_type != 'TYPE_NONE':
            ...
            if obj.type == 'EMPTY':
                errmsg = _z("Empty type objects cannot be set as a FLIP Fluid {}.").format(_z(flip_type))
                errdesc = _z("Empty type objects are only supported as a FLIP Fluid Force Field.")
                bpy.ops.flip_fluid_operators.display_error(
                    'INVOKE_DEFAULT',
                    error_message=errmsg,
                ...
                errmsg = _z("Curve type objects cannot be set as a FLIP Fluid {}.").format(_z(flip_type))
                errdesc = _z("Curve type objects are only supported as a FLIP Fluid Force Field > Curve Guide Force.")
                bpy.ops.flip_fluid_operators.display_error(
                    'INVOKE_DEFAULT',
                    error_message=errmsg,
                    error_description=errdesc,
        ...
        if obj.type == 'FONT' and self.object_type != 'TYPE_NONE':
            errmsg = _z("Text type objects are not supported.")
            errdesc = _z("Text type objects are not supported. Please convert to a mesh object before setting as FLIP Fluid object.")
            bpy.ops.flip_fluid_operators.display_error(
                'INVOKE_DEFAULT',
                error_message=errmsg,
                error_description=errdesc,
        ...
        if obj.type == 'META' and self.object_type != 'TYPE_NONE':
            errmsg = _z("Metaball type objects are not supported.")
            errdesc = _z("Metaball type objects are not supported. Please convert to a mesh object before setting as FLIP Fluid object.")
            bpy.ops.flip_fluid_operators.display_error(
                'INVOKE_DEFAULT',
                error_message=errmsg,
                error_description=errdesc,
        ...
        if obj.type == 'VOLUME' and self.object_type != 'TYPE_NONE':
            errmsg = _z("Volume type objects are not supported.")
            errdesc = _z("Volume type objects are not supported. Please convert to a mesh object before setting as FLIP Fluid object.")
            bpy.ops.flip_fluid_operators.display_error(
                'INVOKE_DEFAULT',
                error_message=errmsg,
                error_description=errdesc,
        ...
        if obj.type == 'ARMATURE' and self.object_type != 'TYPE_NONE':
            errmsg = _z("Armature type objects are not supported.")
            errdesc = _z("Armature type objects are not supported. Use the mesh associated with the armature before setting as FLIP Fluid object.")
            bpy.ops.flip_fluid_operators.display_error(
                'INVOKE_DEFAULT',
                error_message=errmsg,
                error_description=errdesc,
        ...
        if obj.type == 'LIGHT' and self.object_type != 'TYPE_NONE':
            errmsg = _z("Light type objects are not supported.")
            errdesc = _z("Light objects are not supported as a FLIP Fluid object type.")
            bpy.ops.flip_fluid_operators.display_error(
                'INVOKE_DEFAULT',
                error_message=errmsg,
                error_description=errdesc,
        ...
        if obj.type == 'LIGHT_PROBE' and self.object_type != 'TYPE_NONE':
            errmsg = _z("Light Probe type objects are not supported.")
            errdesc = _z("Light Probe objects are not supported as a FLIP Fluid object type.")
            bpy.ops.flip_fluid_operators.display_error(
                'INVOKE_DEFAULT',
                error_message=errmsg,
                error_description=errdesc,
        ...
        if obj.type == 'GPENCIL' and self.object_type != 'TYPE_NONE':
            errmsg = _z("Grease Pencil type objects are not supported.")
            errdesc = _z("Grease Pencil objects are not supported as a FLIP Fluid object type.")
            bpy.ops.flip_fluid_operators.display_error(
                'INVOKE_DEFAULT',
                error_message=errmsg,
                error_description=errdesc,
        ...
        if obj.type == 'LATTICE' and self.object_type != 'TYPE_NONE':
            errmsg = _z("Lattice type objects are not supported.")
            errdesc = _z("Lattice objects are not supported as a FLIP Fluid object type.")
            bpy.ops.flip_fluid_operators.display_error(
                'INVOKE_DEFAULT',
                error_message=errmsg,
                error_description=errdesc,
        ...
        if obj.type == 'CAMERA' and self.object_type != 'TYPE_NONE':
            errmsg = _z("Camera type objects are not supported.")
            errdesc = _z("Camera objects are not supported as a FLIP Fluid object type.")
            bpy.ops.flip_fluid_operators.display_error(
                'INVOKE_DEFAULT',
                error_message=errmsg,
                error_description=errdesc,
        ...
        if obj.type == 'SPEAKER' and self.object_type != 'TYPE_NONE':
            errmsg = _z("Speaker type objects are not supported.")
            errdesc = _z("Speaker objects are not supported as a FLIP Fluid object type.")
            bpy.ops.flip_fluid_operators.display_error(
                'INVOKE_DEFAULT',
                error_message=errmsg,
                error_description=errdesc,
