...
class FlipFluidProperties(bpy.types.PropertyGroup):
    ...
    show_render = BoolProperty(
            name=_z("Show Render"),
            description="Show Render\nDisplay simulation in render. If disabled, simulation data will not be loaded in the render",
            default=True,
            ); exec(conv("show_render"))
    show_viewport = BoolProperty(
            name=_z("Show Viewport"),
            description="Show Viewport\nDisplay simulation in viewport. If disabled, simulation data will not be loaded in the viewport."
                " Disable to speed up playback while working on other areas of your scene",
            default=True,
            ); exec(conv("show_viewport"))

    logo_name = StringProperty(_z("flip_fluids_logo"), description = "flip_fluids_logo"); exec(conv("logo_name"))
    ...
    def register(cls):
        bpy.types.Scene.flip_fluid = PointerProperty(
                name=_z("Flip Fluid Properties"),
                description="Flip Fluid Properties",
                type=cls,
                )
