...
class DomainBakeProperties(bpy.types.PropertyGroup):
    ...
    original_frame_start = IntProperty(
            name=_z("Start Frame"),
            description="Start Frame\nFirst frame of the simulation cache. Cannot be changed"
    ...
    def _update_savestate_enums(self):
        ...
        for frameno in savestate_frames:
            name =_z("Resume from frame {}").format(str(frameno + 1)) 
            if frameno > autosave_frame:
                name += _z(" (outdated)")
            e = (str(frameno), name, "")
            SAVESTATE_ENUMS.append(e)
        e = (str(autosave_frame), _z("Resume from frame {} (most recent)").format(autosave_frame + 1), "")
        SAVESTATE_ENUMS.append(e)
