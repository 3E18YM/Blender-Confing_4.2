...
def get_command_line_script_filepath(script_filename):
    ...
    if not os.path.isfile(script_path):
        errmsg = _z("Unable to locate script <{}>. Please contact the developers with this error.").format(script_path)
...
def launch_command_darwin_or_linux(command_text, script_prefix_string):
    ...
    if system == "Darwin":
        ...
        if shutil.which("gnome-terminal") is not None and shutil.which("bash") is not None:
            ...
            errmsg = _z("This feature requires the (GNOME Terminal and Bash Shell), or the XTERM terminal emulator to be installed and to be accessible on the system path. ")
            errmsg += _z("Either install these programs, restart Blender, ")
            errmsg += _z("and try again or use the Copy Command to Clipboard operator and paste into a terminal program of your choice.")
            bpy.ops.flip_fluid_operators.display_error(
                'INVOKE_DEFAULT',
                error_message=_z("Linux: Unable to launch new terminal window"),
...
class FlipFluidHelperCommandLineBake(bpy.types.Operator):
    ...
    def check_and_report_operator_context_errors(self, context):
        ...
        if not context.scene.flip_fluid.is_domain_in_active_scene():
            self.report({"ERROR"}, 
                        _z("Active scene must contain domain object to launch bake. Select the scene that contains the domain object, save, and try again."))
            return {'CANCELLED'}
        ...
        if self.is_render_output_format_image_required(context) and not is_render_output_format_image():
            self.report({'ERROR'}, _z("Render output format must be an image format for this render mode. Change render output to an image, save, and try again."))
            return {'CANCELLED'}

        if platform.system() not in ["Windows", "Darwin", "Linux"]:
            self.report({'ERROR'}, _z("System platform <{}> not supported. This feature only supports Windows, MacOS, or Linux system platforms.").format(platform.system()))
            return {'CANCELLED'}
    ...
    def execute(self, context):
        ...
        info_msg = _z("Launched command line baking window. If the baking process did not begin,")
        info_msg += _z(" this may be caused by a conflict with another addon or a security feature of your OS that restricts automatic command execution.")
        info_msg += _z(" You may try copying the following command manually into a command line window:\n\n")
        info_msg += command_text + "\n\n"
        info_msg += _z("For more information on command line baking, visit our documentation:\n")
...
class FlipFluidHelperCommandLineBakeToClipboard(bpy.types.Operator):
    ...
    bl_label = _z("Copy Bake Command to Clipboard")
    ...
    def execute(self, context):
        ...
        info_msg = _z("Copied the following baking command to your clipboard:\n\n")
        info_msg += command_text + "\n\n"
        info_msg += _z("For more information on command line baking, visit our documentation:\n")
        info_msg += "https://github.com/rlguy/Blender-FLIP-Fluids/wiki/Helper-Menu-Settings#command-line-bake"
        self.report({'INFO'}, info_msg)
...
class FlipFluidHelperCommandLineRender(bpy.types.Operator):
    ...
    def check_and_report_operator_context_errors(self, context):
        if not is_render_output_directory_createable():
            errmsg = _z("Render output directory is not valid or writeable: <{}>").format(get_render_output_directory())
            self.report({'ERROR'}, errmsg)
            return {'CANCELLED'}
    ...
    def execute(self, context):
        ...
        info_msg = _z("Launched command line render window. If the render process did not begin,")
        info_msg += _z(" this may be caused by a conflict with another addon or a security feature of your OS that restricts automatic command execution. ")
        info_msg += _z("You may try copying the following command manually into a command line window:\n\n")
        info_msg += command_text + "\n\n"
        info_msg += _z("For more information on command line rendering, visit our documentation:\n")
...
class FlipFluidHelperCommandLineRenderToClipboard(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        info_msg = _z("Copied the following render command to your clipboard:\n\n")
        info_msg += command_text + "\n\n"
        info_msg += _z("For more information on command line rendering, visit our documentation:\n")
        info_msg += "https://github.com/rlguy/Blender-FLIP-Fluids/wiki/Helper-Menu-Settings#command-line-animation-render"
        self.report({'INFO'}, info_msg)
...
class FlipFluidHelperCommandLineRenderFrame(bpy.types.Operator):
    ...
    bl_label = _z("Launch Frame Render")
    ...
    def check_and_report_operator_context_errors(self, context):
        ...
        if not is_render_output_format_image():
            self.report({'ERROR'}, _z("Render output format must be an image format. Change render output to an image, save, and try again."))
            return {'CANCELLED'} 
...
class FlipFluidHelperCommandLineAlembicExport(bpy.types.Operator):
    ...
    bl_label = _z("Launch Alembic Export")
    ...
    def execute(self, context):
        ...
        info_msg = _z("Launched command line Alembic export window. If the Alembic export process did not begin,")
        info_msg += _z(" this may be caused by a conflict with another addon or a security feature of your OS that restricts automatic command execution. ")
        info_msg += _z("You may try copying the following command manually into a command line window:\n\n")
        info_msg += command_text + "\n\n"
        info_msg += _z("For more information on command line operators, visit our documentation:\n")
...
class FlipFluidHelperCmdAlembicExportToClipboard(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        info_msg = _z("Copied the following Alembic export command to your clipboard:\n\n")
        info_msg += command_text + "\n\n"
        info_msg += _z("For more information on command line tools, visit our documentation:\n")
        info_msg += "https://github.com/rlguy/Blender-FLIP-Fluids/wiki/Helper-Menu-Settings#command-line-alembic-export"
        self.report({'INFO'}, info_msg)
...
class FlipFluidHelperCommandLineRenderToScriptfile(bpy.types.Operator):
    ...
    bl_label = _z("Generate Batch File")
    ...
    def execute(self, context):
        ...
        more_info_string = _z("For more information on batch rendering, visit our documentation:\n")
        more_info_string += "https://github.com/rlguy/Blender-FLIP-Fluids/wiki/Helper-Menu-Settings#command-line-animation-render\n"
        render_output_info_string = _z("View the rendered files at <{}>").format(directory_path)
        ...
        if not missing_frames:
            info_msg = _z("No batch file generated! All frames have already been rendered.\n")
        ...
        info_msg = _z("\nA batch file has been generated here: <{}>").format(batch_filepath)
        info_msg += render_output_info_string + "\n"
        info_msg += _z("{} frames in the {} file format have already been rendered!\n").format((total_frames - len(missing_frames)), file_suffix)
        info_msg += str(len(missing_frames)) + _z(" frames are not yet rendered!\n\n")
        info_msg += more_info_string
...
class FlipFluidHelperRunScriptfile(bpy.types.Operator):
    ...
    bl_label = _z("Launch Batch File Render")
    ...
    def execute(self, context):
        ...
        if self.regenerate_batch_file or not os.path.isfile(batch_filepath):
            ...
            if not os.path.isfile(batch_filepath):
                self.report({'ERROR'}, _z("Unable to generate the render script."))

        os.startfile(batch_filepath)
          
        info_msg = _z("Beginning to run the batch render script!\n\n")
        info_msg += _z("For more information on batch file rendering, visit our documentation:\n")
        info_msg += "https://github.com/rlguy/Blender-FLIP-Fluids/wiki/Helper-Menu-Settings#command-line-animation-render"
        self.report({'INFO'}, info_msg)
        return {'FINISHED'}
class FlipFluidHelperOpenOutputFolder(bpy.types.Operator):
    bl_idname = "flip_fluid_operators.helper_open_outputfolder"
    bl_label = _z("Opens The Output Folder")
...
class FlipFluidHelperCommandLineRenderPassAnimation(bpy.types.Operator):
    ...
    bl_label = _z("Launch Render Pass Animation")
    bl_description = "Launch Render Pass Animation\nDescription: todo - launch render pass animation script"
    ...
    def check_and_report_operator_context_errors(self, context):
        ...
        if not context.scene.flip_fluid.is_domain_in_active_scene():
            self.report({"ERROR"},
                        _z("Active scene must contain domain object to launch render. Select the scene that contains the domain object, save, and try again."))
            return {'CANCELLED'}
        ...
        if context.scene.render.engine == 'BLENDER_EEVEE':
            self.report({'ERROR'}, _z("The EEVEE render engine is not supported for this feature. Set the render engine to Cycles, save, and try again."))
            return {'CANCELLED'}
        if context.scene.render.engine == 'BLENDER_WORKBENCH':
            self.report({'ERROR'}, _z("The Workbench render engine is not supported for this feature. Set the render engine to Cycles, save, and try again."))
            return {'CANCELLED'}

        if not is_render_output_format_image_with_transparency():
            errmsg = _z("Render output format must be an image format that supports transparency.")
            errmsg += _z(" The OpenEXR format is recommended.")
            errmsg += _z(" Change render output to an image format with transparency, save, and try again.")
            self.report({'ERROR'}, errmsg)
            return {'CANCELLED'}

        if not is_render_output_format_image_set_to_RGBA_color_mode():
            errmsg = _z("Render output format color mode must be set to RGBA for transparency.")
            errmsg += _z(" The current color mode is set to <{}>.").format(bpy.context.scene.render.image_settings.color_mode) 
            errmsg += _z(" Change render output color mode to RGBA, save, and try again.")
            self.report({'ERROR'}, errmsg)
            return {'CANCELLED'}
...
class FlipFluidHelperCommandLineRenderPassAnimationToClipboard(bpy.types.Operator):
    ...
    bl_label = _z("Copy Render Pass Animation Command to Clipboard")
    bl_description = "Copy Render Pass Animation Command to Clipboard\nDescription: todo - launch render pass animation script to clipboard"
...
class FLIPFLUIDS_MT_render_menu(bpy.types.Menu):
    bl_label = _z("FLIP Fluids CMD Render")
    bl_description = "FLIP Fluids CMD Render"
    ...
    def draw(self, context):
        ...
        row2.operator(FlipFluidHelperCommandLineRender.bl_idname, text=_z("Launch Animation Render"), icon="RENDER_ANIMATION").use_turbo_tools=False
        if system == "Windows":
            row3.operator(FlipFluidHelperRunScriptfile.bl_idname, text=_z("Launch Batch Render"), icon="CONSOLE").regenerate_batch_file=True
