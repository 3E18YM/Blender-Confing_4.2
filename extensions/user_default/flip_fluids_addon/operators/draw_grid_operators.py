...
class FlipFluidDrawDebugGrid(bpy.types.Operator):
    ...
    bl_label = _z("Draw Debug Grid")
    bl_description = "Draw Debug Grid\nDraw debug view of the domain simulation grid"
    bl_options = {'REGISTER'}
    ...
    def draw_callback_2d(self, context):
        ...
        if dprops.debug.grid_display_mode == 'GRID_DISPLAY_MESH':
            ...
            isize, jsize, ksize, simulation_dx = dprops.simulation.get_viewport_grid_dimensions()
            reduction = 1
            force_field_quality_str = _z('Ultra')
            if dprops.world.force_field_resolution == 'FORCE_FIELD_RESOLUTION_LOW':
                reduction = 4
                force_field_quality_str = _z('Low')
            elif dprops.world.force_field_resolution == 'FORCE_FIELD_RESOLUTION_NORMAL':
                reduction = 3
                force_field_quality_str = _z('Normal')
            elif dprops.world.force_field_resolution == 'FORCE_FIELD_RESOLUTION_HIGH':
                reduction = 2
                force_field_quality_str = _z('High')
            isize = int(math.ceil(isize / reduction))
            jsize = int(math.ceil(jsize / reduction))
            ksize = int(math.ceil(ksize / reduction))
        ...
        if dprops.debug.grid_display_mode == 'GRID_DISPLAY_SIMULATION':
            ...
            blf.draw(font_id, _z("Simulation Grid"))

            blf.size(font_id, 15, 72)
            blf.position(font_id, xstart + 10, height - 80, 0)
            blf.draw(font_id, _z("Grid Resolution: {} x {} x {}").format(str(isize), str(jsize), str(ksize)))
            ...
            blf.draw(font_id, _z("Grid Dimensions: {}m x {}m x {}m").format(str(dimx), str(dimy), str(dimz)))

            blf.position(font_id, xstart + 10, height - 130, 0)
            blf.draw(font_id, _z("Voxel Count: ") + format(isize*jsize*ksize, ",").replace(",", " "))

            blf.position(font_id, xstart + 10, height - 155, 0)
            blf.draw(font_id, _z("Voxel Width: {}m").format(str(round(simulation_dx, 4))))
        elif dprops.debug.grid_display_mode == 'GRID_DISPLAY_MESH':
            ...
            blf.size(font_id, 20, 72)
            blf.position(font_id, xstart, height - 50, 0)
            blf.draw(font_id, _z("Final Surface Mesh Grid"))

            blf.size(font_id, 15, 72)
            blf.position(font_id, xstart + 10, height - 80, 0)
            blf.draw(font_id, _z("Subdivisions: ") + str(dprops.surface.subdivisions))

            blf.position(font_id, xstart + 10, height - 105, 0)
            blf.draw(font_id, _z("Compute Chunks: ") + str(compute_chunks))

            blf.position(font_id, xstart + 10, height - 130, 0)
            blf.draw(font_id, _z("Grid Resolution: ") + str(isize) + " x " + str(jsize) + " x " + str(ksize))
            ...
            blf.position(font_id, xstart + 10, height - 155, 0)
            blf.draw(font_id, _z("Voxel Count: {} ({} / chunk)").format(num_cells_str, chunk_cells_str))

            blf.position(font_id, xstart + 10, height - 180, 0)
            blf.draw(font_id, _z("Voxel Width: ") + str(round(simulation_dx, 4)))
        elif dprops.debug.grid_display_mode == 'GRID_DISPLAY_PREVIEW':
            blf.size(font_id, 20, 72)
            blf.position(font_id, xstart, height - 50, 0)
            blf.draw(font_id, _z("Preview Surface Mesh Grid"))
            ...
            blf.size(font_id, 20, 72)
            blf.position(font_id, xstart, height - 50, 0)
            blf.draw(font_id, _z("Force Field Grid"))

            blf.size(font_id, 15, 72)
            blf.position(font_id, xstart + 10, height - 80, 0)
            blf.draw(font_id, _z("Force Field Quality: ") + force_field_quality_str)

            blf.position(font_id, xstart + 10, height - 105, 0)
            blf.draw(font_id, _z("Force Field Resolution: ") + str(isize) + " x " + str(jsize) + " x " + str(ksize))
