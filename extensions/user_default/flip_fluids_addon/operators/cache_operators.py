...
class FlipFluidFreeCache(bpy.types.Operator):
    ...
    bl_label = _z("Free Cache")
    bl_description = "Free Cache\nDelete Simulation Cache Files"
    ...
    def execute(self, context):
        ...
        if not os.path.isdir(cache_path):
            self.report({"ERROR"}, _z("Current cache directory does not exist"))
            return {'CANCELLED'}

        self.clear_cache(context)
        self.report({"INFO"}, _z("Successfully cleared cache directory '{}'.").format(cache_path))
...
class FlipFluidFreeUnheldCacheFiles(bpy.types.Operator):
    ...
    bl_label = _z("Free Unheld Cache Files")
    bl_description = _z("Delete Unheld Simulation Cache Files")
    ...
    def execute(self, context):
        ...
        self.report({"INFO"}, _z("Successfully cleared unheld cache files"))
...
class FlipFluidMoveCache(bpy.types.Operator):
    ...
    bl_label = _z("Move Cache")
    bl_description = "Move Cache\nMove Simulation Cache Files"
    ...
    def execute(self, context):
        ...
        try:
            ...
            self.report({"ERROR"}, _z("Error creating cache directory: ") + str(e))
            return {'CANCELLED'}
        ...
        try:
            ...
            self.report({"ERROR"}, _z("Error creating destination directory: ") + str(e))
            return {'CANCELLED'}

        if not os.access(dst_dir, os.W_OK):
            self.report({"ERROR"}, _z("Unable to write to destination directory"))
            return {'CANCELLED'}

        try:
            shutil.move(src_dir, dst_dir)
        except Exception as e:
            self.report({"ERROR"}, _z("Error moving cache directory: ") + str(e))
            return {'CANCELLED'}

        base_dir = os.path.basename(src_dir)
        new_cache_path = os.path.join(dst_dir, base_dir)
        self.report({"INFO"}, _z("Successfully moved '{}' to '{}'.").format(src_dir, new_cache_path))
...
class FlipFluidRenameCache(bpy.types.Operator):
    ...
    bl_label = _z("Rename Cache")
    bl_description = "Rename Cache\nRename Simulation Cache Directory"
    ...
    def execute(self, context):
        ...
        if os.path.exists(new_cache_path):
            self.report({"ERROR"}, _z("Renamed cache directory already exists"))
            return {'CANCELLED'}
        if not os.access(os.path.dirname(new_cache_path), os.W_OK):
            dst_dir = os.path.dirname(new_cache_path)
            self.report({"ERROR"}, _z("Unable to write to destination directory: ") + dst_dir)
            return {'CANCELLED'}

        try:
            os.rename(src_dir, new_cache_path)
        except Exception as e:
            self.report({"ERROR"}, _z("Error renaming cache directory: ") + str(e))
            return {'CANCELLED'}

        self.report({"INFO"}, _z("Successfully renamed '{}' to '{}'.").format(src_dir, new_cache_path))
...
class FlipFluidCopyCache(bpy.types.Operator):
    ...
    bl_label = _z("Copy Cache")
    bl_description = "Copy Cache\nCopy Simulation Cache Directory"
    ...
    def execute(self, context):
        ...
        try:
            ...
            self.report({"ERROR"}, _z("Error copying cache directory: ") + str(e))
            return {'CANCELLED'}

        self.report({"INFO"}, _z("Successfully copied '{}' to '{}'.").format(src_dir, dst_dir))
...
class FlipFluidRelativeCacheDirectory(bpy.types.Operator):
    ...
    bl_label = _z("Make Relative")
    bl_description = "Make Relative\nConvert to a filepath relative to the Blend file"
    ...
    def execute(self, context):
        ...
        try:
            ...
            info_msg =  _z("Relative path requires Blend file and cache directory to be on the same drive")
            info_msg += _z(" Resetting cache to default relative path.")
...
class FlipFluidAbsoluteCacheDirectory(bpy.types.Operator):
    ...
    bl_label = _z("Make Absolute")
    bl_description = "Make Absolute\nConvert to an absolute filepath location"
...
class FlipFluidMatchFilenameCacheDirectory(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        blend_filepath = bpy.path.abspath("//")
        if not blend_filepath:
            self.report({"ERROR"}, _z("The Blend file must be saved to use this operator"))
            return {'CANCELLED'}
...
class FlipFluidIncreaseDecreaseCacheDirectory(bpy.types.Operator):
    ...
    bl_label = _z("Increase/Decrease Cache Directory")
...
class FlipFluidIncreaseDecreaseRenderDirectory(bpy.types.Operator):
    ...
    bl_label = _z("Increase/Decrease Render Directory")
...
class FlipFluidIncreaseDecreaseCacheRenderVersion(bpy.types.Operator):
    ...
    bl_label = _z("Increase/Decrease Cache and Render Version")
...
class FlipFluidRelativeLinkedGeometryDirectory(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if not linked_directory:
            self.report({"ERROR"}, _z("Linked geometry directory is not set. Set to an existing cache directory and try again."))
            return {'CANCELLED'}

        try:
            relpath = os.path.relpath(linked_directory, blend_filepath)
        except ValueError:
            self.report({"ERROR"}, _z("Relative path requires Blend file and cache directory to be on the same drive"))
            return {'CANCELLED'}
...
class FlipFluidClearLinkedGeometryDirectory(bpy.types.Operator):
    ...
    bl_label = _z("Clear")
    bl_description = "Clear\nClear the linked geometry directory field. No files will be deleted"
