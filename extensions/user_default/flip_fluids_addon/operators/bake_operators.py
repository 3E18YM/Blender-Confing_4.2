...
class BakeFluidSimulation(bpy.types.Operator):
    ...
    bl_label = _z("Bake Fluid Simulation")
    bl_description = "Bake Fluid Simulation\nRun fluid simulation"
    bl_options = {'REGISTER'}
        ...
        if self.thread and not self.thread.is_alive():
            ...
            if self.data.error_message:
                bpy.ops.flip_fluid_operators.display_error(
                    'INVOKE_DEFAULT',
                    error_message=_z("Error Baking Fluid Simulation"),
    ...
    def execute(self, context):
        if not installation_utils.is_installation_complete():
            self.report({"ERROR"}, 
                         _z("FLIP Fluids installation incomplete. Restart Blender to complete installation. If you think this is an error, please contact the developers."))
            self.cancel(context)
            return {'CANCELLED'}

        if not context.scene.flip_fluid.is_domain_object_set():
            self.report({"ERROR_INVALID_INPUT"}, 
                         _z("Fluid simulation requires a domain object"))
            self.cancel(context)
            return {'CANCELLED'}

        if context.scene.flip_fluid.get_num_domain_objects() > 1:
            self.report({"ERROR_INVALID_INPUT"}, 
                        _z("There must be only one domain object in the Blend file"))
            self.cancel(context)
            return {'CANCELLED'}

        if not context.scene.flip_fluid.is_domain_in_active_scene():
            self.report({"ERROR"}, 
                         _z("Active scene must contain domain object to begin baking. Select the scene that contains the domain object and try again."))
            self.cancel(context)
            return {'CANCELLED'}
        ...
        if not os.path.exists(cache_directory):
            try:
                ...
                msg = _z("Unable to create cache directory: <{}> ").format(cache_directory)
                msg += _z("Set the cache directory in the 'Domain > FLIP Fluid Cache panel' to a location on your system with write permissions.")
...
class BakeFluidSimulationCommandLine(bpy.types.Operator):
    ...
    bl_label = _z("Bake Fluid Simulation")
    bl_description = "Bake Fluid Simulation\nBake fluid simulation from command line"
    bl_options = {'REGISTER'}
    ...
    def execute(self, context):
        ...
        if context.scene.flip_fluid.get_num_domain_objects() > 1:
            self.report({"ERROR_INVALID_INPUT"}, 
                        _z("There must be only one domain object"))
            self.cancel(context)
            return {'CANCELLED'}

        if not context.scene.flip_fluid.is_domain_in_active_scene():
            self.report({"ERROR"}, 
                        _z("Active scene must contain domain object to begin baking. Select the scene that contains the domain object, save, and try again."))
            self.cancel(context)
            return {'CANCELLED'}
...
class CancelBakeFluidSimulation(bpy.types.Operator):
    ...
    bl_label = _z("Cancel Bake Fluid Simulation")
    bl_description = "Cancel Bake Fluid Simulation\nStop baking fluid simulation"
...
class FlipFluidResetBake(bpy.types.Operator):
    ...
    bl_label = _z("Reset Bake")
    bl_description = _z("Reset simulation bake to initial state\nWARNING: this operation will delete previously baked simulation data.")
    ...
    def execute(self, context):
        ...
        if not os.path.isdir(cache_path):
            ...
            self.report({"INFO"}, _z("Current cache directory does not exist - skipping cache reset"))
            return {'FINISHED'}
        ...
        self.report({"INFO"}, _z("Successfully reset bake"))
        return {'FINISHED'}
