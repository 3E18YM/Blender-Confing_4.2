...
class FlipFluidImportMaterial(bpy.types.Operator):
    ...
    bl_label = _z("Import")
    bl_description = "Import\nImport the selected material and link to material library"
    bl_options = {'REGISTER'}
    ...
    def import_single_material(self, library_name):
        ...
        if is_imported:
            msg = _z("Library material already imported: <{}>").format(library_name)
            if library_name != imported_material_name:
                msg += _z(" as <{}>").format(imported_material_name)
            self.report({'INFO'}, msg)
            return {'FINISHED'}

        imported_material_name = material_library.import_material(library_name)

        msg = _z("Successfully imported library material: <{}>").format(library_name)
        if library_name != imported_material_name:
            msg += _z(" as <{}>").format(imported_material_name)
        self.report({'INFO'}, msg)
...
class FlipFluidImportMaterialCopy(bpy.types.Operator):
    ...
    bl_label = _z("Import Copy")
    bl_description = "Import Copy\nImport a copy of the selected material and do not link to material library"
    bl_options = {'REGISTER'}
    ...
    def import_single_material(self, library_name):
        ...
        msg = _z("Successfully imported copy of library material: <{}>").format(library_name)
        if library_name != imported_name:
            msg += _z(" as <{}>").format(imported_name)
