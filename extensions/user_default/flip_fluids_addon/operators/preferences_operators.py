...
class FLIPFluidPreferencesExportUserData(bpy.types.Operator):
    ...
    bl_label = _z("Export User Settings")
    ...
    def execute(self, context):
        if os.path.isdir(self.filepath):
            errmsg = _z("Error: file path is already a directory")
            desc = _z("Path: <{}>").format(self.filepath)
        ...
        if error:
            errmsg = _z("Error: unable to export user settings")
            bpy.ops.flip_fluid_operators.display_error(
                    'INVOKE_DEFAULT',
                    error_message=errmsg,
        ...
        self.report({'INFO'}, _z("Successfully exported user settings to: <{}>").format(self.filepath))
        return {'FINISHED'}
...
class FLIPFluidPreferencesImportUserData(bpy.types.Operator, ImportHelper):
    ...
    bl_label = _z("Import User Settings")
    bl_description = "Import User Settings\nLoad user settings and presets from a previous installation"
    ...
    def execute(self, context):
        ...
        if error:
            errmsg = _z("Error: unable to import user settings")
            bpy.ops.flip_fluid_operators.display_error(
                    'INVOKE_DEFAULT',
                    error_message=errmsg,
                    error_description=error,
...
class FLIPFluidInstallMixboxPlugin(bpy.types.Operator, ImportHelper):
    ...
    def execute(self, context):
        ...
        if not os.path.exists(self.filepath):
            self.report_error_message(context, _z("Error: File does not exist. Select the Mixbox.plugin file."))
            return {'CANCELLED'}

        if not os.path.isfile(self.filepath):
            self.report_error_message(context, _z("Error: No file selected. Select the Mixbox.plugin file."))
            return {'CANCELLED'}
        ...
        try:
            ...
            with zipfile.ZipFile(self.filepath, 'r') as zip:
                if not expected_lut_filename in zip.namelist():
                    self.report_error_message(context, _z("Error: Invalid plugin contents. File may be corrupted."))
                    return {'CANCELLED'}

                for f in zip.infolist():
                    if f.filename == expected_lut_filename and f.file_size != expected_lut_filesize:
                        self.report_error_message(context, _z("Error: Invalid plugin data. File may be corrupted."))
                        return {'CANCELLED'}
            ...
            self.report_error_message(context, _z("Error: Invalid plugin installation file."))
            return {'CANCELLED'}
        except Exception as e:
            self.report_error_message(context, _z("Unknown Error Encountered: ") + str(e))
            return {'CANCELLED'}

        installation_utils.update_mixbox_installation_status()
        success_message = _z("The Mixbox plugin has been installed successfully.")
...
class FLIPFluidUninstallMixboxPlugin(bpy.types.Operator):
    ...
    bl_label = _z("Uninstall Mixbox")
    bl_description = "Uninstall Mixbox\nUninstall the Mixbox plugin"
    ...
    def execute(self, context):
        ...
        installation_utils.update_mixbox_installation_status()
        success_message = _z("The Mixbox plugin has been uninstalled successfully.")
        self.report({'INFO'}, success_message)
        print(success_message)
        return {'FINISHED'}
...
class FLIPFluidInstallPresetLibrary(bpy.types.Operator, ImportHelper):
    ...
    bl_label = _z("Install Preset Scenes Library")
    ...
    def execute(self, context):
        ...
        if not install_location:
            self.report_error_message(context, _z("Error: No install location selected. Select an install location above."))
            return {'CANCELLED'}

        if not os.path.exists(self.filepath):
            self.report_error_message(context, _z("Error: File does not exist. Select the Preset Scenes zip file."))
            return {'CANCELLED'}

        if not os.path.isfile(self.filepath):
            self.report_error_message(context, _z("Error: No file selected. Select the Preset Scenes zip file."))
            return {'CANCELLED'}

        if os.path.isfile(install_location):
            self.report_error_message(context, _z("Error: Invalid install location. Selected install location is an existing file. Select a valid install directory above."))
            return {'CANCELLED'}
        ...
        if not os.path.isdir(install_location):
            try:
                ...
                self.report_error_message(context, _z("Error: Access denied. Unable to write files to install location: <{}>").format(install_location))
                return {'CANCELLED'}
        ...
        if not os.path.isdir(test_directory):
            ...
            self.report_error_message(context, _z("Error: Unknown Error. This should not happen. Try again or contact the developers."))
            return {'CANCELLED'}
        ...
        try:
            ...
            with zipfile.ZipFile(self.filepath, 'r') as zip:
                if not expected_metadata_filepath in zip.namelist():
                    self.report_error_message(context, _z("Error: Invalid Preset Scenes zip file contents. File may be corrupted."))
                    return {'CANCELLED'}
                ...
                with zip.open(expected_metadata_filepath) as version_json:
                    try:
                        ...
                        self.report_error_message(context, _z("Error: Invalid Preset Metadata <Error: {}>. Contact the developers.").format(str(e)))
                        return {'CANCELLED'}
                    
                zip.extractall(path=install_location)

        except zipfile.BadZipFile as e:
            self.report_error_message(context, _z("Error: Invalid Preset Scenes zip file. File may be corrupted."))
            return {'CANCELLED'}
        except Exception as e:
            self.report_error_message(context, _z("Unknown Error Encountered: {}").format(str(e)))
            return {'CANCELLED'}
...
class FLIPFluidSelectPresetLibraryFolder(bpy.types.Operator):
    ...
    bl_label = _z("Install Preset Folder")
    bl_description = "Install Preset Folder\nSelect an existing Preset Library installation folder and add it to the Blender Asset Browser"

    directory = bpy.props.StringProperty(name=_z("Directory"), options={"HIDDEN"}, description = "Directory")
    ...
    def execute(self, context):
        ...
        if not os.path.exists(install_directory):
            self.report_error_message(context, _z("Error: Selected folder does not exist <{}>").format(install_directory))
            return {'CANCELLED'}

        if not os.path.isdir(install_directory):
            self.report_error_message(context, _z("Error: Selected folder is not a directory <{}>").format(install_directory))
            return {'CANCELLED'}

        if not os.listdir(install_directory):
            self.report_error_message(context, _z("Error: Selected folder is empty <{}>").format(install_directory))
            return {'CANCELLED'}
...
class FLIPFluidPresetLibraryCopyInstallLocation(bpy.types.Operator):
    ...
    bl_label = _z("Copy Install Location")
...
class FLIPFluidUninstallPresetLibrary(bpy.types.Operator):
    ...
    bl_label = _z("Uninstall Preset Library")
    ...
    def execute(self, context):
        ...
        installation_utils.update_preset_library_installation_status()
        success_message = _z("The Preset Library <{}> has been uninstalled successfully.").format(install_info["name"])
        self.report({'INFO'}, success_message)
        self.tag_redraw(context)
...
class FLIPFluidPreferencesFindGPUDevices(bpy.types.Operator):
    ...
    bl_label = _z("Find GPU Devices")
    bl_description = "Find GPU Devices\nSearch for GPU compute devices"
    ...
    def execute(self, context):
        ...
        self.report({'INFO'}, _z("Found {} GPU compute device(s).").format(str(len(devices))))
        return {'FINISHED'}
...
class FlipFluidReportBugPrefill(bpy.types.Operator):
    ...
    bl_label = _z("Report a Bug (GitHub)")
...
class FlipFluidCopySystemInfo(bpy.types.Operator):
    ...
    bl_label = _z("Copy System & Blend Info")
...
class FlipFluidOpenPreferences(bpy.types.Operator):
    ...
    bl_label = _z("FLIP Fluids Preferences")
    bl_description = "FLIP Fluids Preferences\nOpen the FLIP Fluids addon preferences menu"
...
class FlipFluidTestBakeAlarm(bpy.types.Operator):
    ...
    bl_label = _z("Test Alarm")
    bl_description = "Test Alarm\nTest and play the alarm sound"
...
class FLIPFLUIDS_MT_help_menu(bpy.types.Menu):
    bl_label = _z("FLIP Fluids")
    bl_description = "FLIP Fluids"
