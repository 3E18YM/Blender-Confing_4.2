...
class ExportFluidSimulation(bpy.types.Operator):
    ...
    bl_label = _z("Export Fluid Simulation")
    bl_description = "Export Fluid Simulation\nExport fluid simulation data"
    bl_options = {'REGISTER'}
    ...
    def modal(self, context, event):
        dprops = self._get_domain_properties()
        if dprops is None:
            self.report({"ERROR_INVALID_INPUT"}, _z("Export operator requires a domain object"))
            self.cancel(context)
            return {'CANCELLED'}

        if not context.scene.flip_fluid.is_domain_in_active_scene():
            self.report({"ERROR"},
                        _z("Active scene must contain domain object during export - halting export and baking process. Please do not switch active scenes during export."))
            self.cancel(context)
            return {'CANCELLED'}
...
class FlipFluidExportStatsCSV(bpy.types.Operator):
    ...
    bl_label = _z("Export CSV")
    bl_description = "Export CSV\nExport simulation stats to CSV file format"
    ...
    def execute(self, context):
        ...
        if not os.path.isfile(statsfile):
            self.report({"ERROR"}, _z("Missing simulation stats data file: ") + statsfile)
            return {'CANCELLED'}
        ...
        try:
            ...
            self.report({"ERROR"}, _z("Error creating csv file directory: ") + str(e))
            return {'CANCELLED'}
        ...
        self.report({"INFO"}, _z("Successfully exported to CSV: ") + csv_filepath)
        return {'FINISHED'}
