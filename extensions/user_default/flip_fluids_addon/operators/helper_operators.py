...
class FlipFluidHelperRemesh(bpy.types.Operator):
    ...
    bl_label = _z("FLIP Fluids Remesh Collection")
    ...
    def display_convert_to_mesh_popup(self, object_list):
        def draw_func(self, context):
            text_label = _z("The following (") + str(len(object_list)) 
            text_label += _z(") selected objects are required be converted to a mesh type before using this operator:")
            ...
            for obj in object_list:
                column_left.label(text=5*" " + _z("Name: ") + obj.name)
                column_right.label(text=_z("Type: ") + obj.type)
            self.layout.label(text="")
            self.layout.label(text=_z("Solutions: "), icon="INFO")
            self.layout.label(text=5*" " + _z("(1) Convert objects to a Blender mesh object"))
            self.layout.label(text=5*" " + _z("(2) Or disable objects in viewport (outliner monitor icon)"))
            self.layout.label(text=5*" " + _z("(3) Or enable 'Convert Objects to Mesh' option"))

        bpy.context.window_manager.popup_menu(draw_func, title=_z("FLIP Fluids Remesh: Actions Required"), icon='INFO')


    def display_apply_modifiers_popup(self, object_list):
        def draw_func(self, context):
            text_label = _z("The following (") + str(len(object_list)) 
            text_label += _z(") selected objects are required to have modifiers applied before using this operator:")
            self.layout.label(text=text_label)
            column = self.layout.column(align=True)
            split = column.split(align=True)
            column_left = split.column(align=True)
            ...
            for obj in object_list:
                column_left.label(text=5*" " + _z("Name: ") + obj.name)
                column_right.label(text=_z("# Modifiers: ") + str(len(obj.modifiers)))
            self.layout.label(text="")
            self.layout.label(text=_z("Solutions: "), icon="INFO")
            self.layout.label(text=5*" " + _z("(1) Apply modifiers to objects"))
            self.layout.label(text=5*" " + _z("(2) Or disable objects in viewport (outliner monitor icon)"))
            self.layout.label(text=5*" " + _z("(3) Or enable 'Apply Object Modifiers' option"))
    ...
    def execute(self, context):
        ...
        if bpy.context.object.mode != 'OBJECT':
            err_msg = _z("FLIP Fluids Remesh: Viewport must be in Object mode to use this operator. ")
            err_msg += _z(" Current viewport mode: <{}>.").format(bpy.context.object.mode)
        ...
        if active_collection is None:
            self.report({'ERROR'}, _z("FLIP Fluids Remesh: No collection selected. Select a collection to remesh."))
            return {'CANCELLED'}
        ...
        if not valid_collection_objects:
            self.report({'ERROR'}, _z("FLIP Fluids Remesh: No valid objects to remesh."))
            return {'CANCELLED'}
        ...
        info_msg = _z("FLIP Fluids Remesh: Successfully merged and remeshed ") 
        info_msg += _z("{} valid objects into object <{}>").format(valid_object_count, context.view_layer.objects.active.name)
...
class FlipFluidHelperSelectDomain(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if num_domains > 1:
            self.report({'ERROR'}, _z("Error: multiple domain objects found. Only one domain per Blend file is supported. Please remove other domains. Found domain objects: ") + found_domains)
            return {'CANCELLED'}
            ...
            for scene in bpy.data.scenes:
                for obj in scene.objects:
                    if obj.name == domain.name:
                        ...
                        break
            self.report({'ERROR'}, _z("Unable to select domain object. Domain object is contained in another scene. Domain Scene: <{}>, Current Scene: <{}>").format(scene_name, context.scene.name))
            return {'CANCELLED'}
...
class FlipFluidHelperSelectSurface(bpy.types.Operator):
    ...
    bl_label = _z("Select Surface")
    bl_description = "Select Surface\nSelect the fluid surface object"
    ...
    def execute(self, context):
        ...
        if not context.scene.flip_fluid.is_domain_in_active_scene():
            self.report({'ERROR'}, _z("Unable to select Surface object: Domain object is not located in the active scene>"))
            return {'CANCELLED'}
        ...
        if surface_object is None:
            self.report({'INFO'}, _z("Fluid Surface object not found in scene"))
            return {'CANCELLED'}
...
class FlipFluidHelperSelectFluidParticles(bpy.types.Operator):
    ...
    bl_label = _z("Select Fluid Particles")
    bl_description = "Select Fluid Particles\nSelect the fluid particles object"
    ...
    def execute(self, context):
        ...
        if fluid_particles_object is None:
            self.report({'INFO'}, _z("Fluid Particles object not found in scene"))
            return {'CANCELLED'}
...
class FlipFluidHelperSelectFoam(bpy.types.Operator):
    ...
    bl_label = _z("Select Foam")
    bl_description = "Select Foam\nSelect the whitewater foam object"
    ...
    def execute(self, context):
        ...
        if dprops is None:
            return {'CANCELLED'}
        if not context.scene.flip_fluid.is_domain_in_active_scene():
            self.report({'ERROR'}, _z("Unable to select Whitewater Foam object: Domain object is not located in the active scene>"))
            return {'CANCELLED'}
        ...
        if foam_object is None:
            self.report({'INFO'}, _z("Whitewater Foam object not found in scene"))
            return {'CANCELLED'}
...
class FlipFluidHelperSelectBubble(bpy.types.Operator):
    ...
    bl_label = _z("Select Bubble")
    bl_description = "Select Bubble\nSelect the whitewater bubble object"
    ...
    def execute(self, context):
        ...
        if dprops is None:
            return {'CANCELLED'}
        if not context.scene.flip_fluid.is_domain_in_active_scene():
            self.report({'ERROR'}, _z("Unable to select Whitewater Bubble object: Domain object is not located in the active scene>"))
            return {'CANCELLED'}
        ...
        if bubble_object is None:
            self.report({'INFO'}, _z("Whitewater Bubble object not found in scene"))
            return {'CANCELLED'}
...
class FlipFluidHelperSelectSpray(bpy.types.Operator):
    ...
    bl_label = _z("Select Spray")
    bl_description = "Select Spray\nSelect the whitewater spray object"
    ...
    def execute(self, context):
        ...
        if dprops is None:
            return {'CANCELLED'}
        if not context.scene.flip_fluid.is_domain_in_active_scene():
            self.report({'ERROR'}, _z("Unable to select Whitewater Spray object: Domain object is not located in the active scene>"))
            return {'CANCELLED'}
        ...
        if spray_object is None:
            self.report({'INFO'}, _z("Whitewater Spray object not found in scene"))
            return {'CANCELLED'}
...
class FlipFluidHelperSelectDust(bpy.types.Operator):
    ...
    bl_label = _z("Select Dust")
    bl_description = "Select Dust\nSelect the whitewater dust object"
    ...
    def execute(self, context):
        ...
        if dprops is None:
            return {'CANCELLED'}
        if not context.scene.flip_fluid.is_domain_in_active_scene():
            self.report({'ERROR'}, _z("Unable to select Whitewater Dust object: Domain object is not located in the active scene>"))
            return {'CANCELLED'}
        ...
        if dust_object is None:
            self.report({'INFO'}, _z("Whitewater Dust object not found in scene"))
            return {'CANCELLED'}
...
class FlipFluidHelperSelectObjects(bpy.types.Operator):
    ...
    bl_label = _z("Select Objects")
    bl_description = "Select Objects\nSelect all FLIP Fluid objects of this type"

    object_type = StringProperty("TYPE_NONE", description = "TYPE_NONE")
...
class FlipFluidHelperCreateDomain(bpy.types.Operator):
    ...
    bl_label = _z("Create Domain")
    bl_description = "Create Domain\nGenerate a domain object for your scene"
    ...
    def set_object_as_domain(self, active_object):
        if active_object.flip_fluid.is_active and active_object.flip_fluid.object_type == 'TYPE_DOMAIN':
            self.report({'ERROR'}, _z("This object is already set as a FLIP Fluid Domain"))
            return
    ...
    def execute(self, context):
        if bpy.context.scene.flip_fluid.is_domain_object_set():
            self.report({'ERROR'}, _z("Scene already contains a domain object"))
            return {'CANCELLED'}
...
class FlipFluidHelperAddObjects(bpy.types.Operator):
    ...
    bl_label = _z("Add Objects")
    bl_description = "Add Objects\nAdd selected objects as FLIP Fluid objects"

    object_type = StringProperty(_z("TYPE_NONE"), description = "TYPE_NONE")
    exec(vcu.convert_attribute_to_28("object_type"))
...
class FlipFluidHelperRemoveObjects(bpy.types.Operator):
    ...
    bl_label = _z("Remove Objects")
    bl_description = "Remove Objects\nRemove selected objects from FLIP Fluid simulation"
...
class FlipFluidHelperDeleteSurfaceObjects(bpy.types.Operator):
    ...
    bl_label = _z("Delete Fluid Surface Mesh Objects")
    ...
    def execute(self, context):
        ...
        self.report({'INFO'}, _z("Deleted fluid surface mesh object"))
        return {'FINISHED'}
...
class FlipFluidHelperDeleteParticleObjects(bpy.types.Operator):
    ...
    bl_label = _z("Delete Fluid Particle Mesh Objects")
    ...
    def execute(self, context):
        ...
        self.report({'INFO'}, _z("Deleted fluid particle mesh object"))
        return {'FINISHED'}
...
class FlipFluidHelperDeleteWhitewaterObjects(bpy.types.Operator):
    ...
    bl_label = _z("Delete Whitewater Mesh Objects")
    ...
    whitewater_type = StringProperty("TYPE_ALL", description = "TYPE_ALL")
    ...
    def execute(self, context):
        ...
        if self.whitewater_type == "TYPE_ALL":
            self.report({'INFO'}, _z("Deleted foam, bubble, spray, and dust whitewater mesh objects"))
        elif self.whitewater_type == "TYPE_FOAM":
            self.report({'INFO'}, _z("Deleted foam whitewater mesh objects"))
        elif self.whitewater_type == "TYPE_BUBBLE":
            self.report({'INFO'}, _z("Deleted bubble whitewater mesh objects"))
        elif self.whitewater_type == "TYPE_SPRAY":
            self.report({'INFO'}, _z("Deleted spray whitewater mesh objects"))
        elif self.whitewater_type == "TYPE_DUST":
            self.report({'INFO'}, _z("Deleted dust whitewater mesh objects"))
...
class FlipFluidHelperOrganizeOutliner(bpy.types.Operator):
    ...
    bl_label = _z("Organize Outliner")
    bl_description = "Organize Outliner\nOrganize simulation objects into separate collections based on FLIP Fluid object type"
...
class FlipFluidHelperSeparateFLIPMeshes(bpy.types.Operator):
    ...
    bl_label = _z("FLIP Meshes to Collections")
...
class FlipFluidHelperUndoOrganizeOutliner(bpy.types.Operator):
    ...
    bl_label = _z("Unlink FLIP Object Collections")
...
class FlipFluidHelperUndoSeparateFLIPMeshes(bpy.types.Operator):
    ...
    bl_label = _z("Unlink FLIP Mesh Collections")
...
class FlipFluidHelperSetObjectViewportDisplay(bpy.types.Operator):
    ...
    bl_label = _z("Object Viewport Display")
    bl_description = "Object Viewport Display\nSet how selected objects are displayed/rendered in the viewport"

    display_mode = StringProperty("TYPE_NONE", description = "TYPE_NONE")
...
class FlipFluidHelperSetObjectRenderDisplay(bpy.types.Operator):
    ...
    bl_label = _z("Object Render Display")
    bl_description = "Object Render Display\nSet selected objects visiblility in the render"
...
class FlipFluidHelperLoadLastFrame(bpy.types.Operator):
    ...
    bl_label = _z("Load Last Frame")
    bl_description = "Load Last Frame\nLoad the most recently computed frame"
...
class FlipFluidEnableFluidParticleOutput(bpy.types.Operator):
    ...
    bl_label = _z("Enable Fluid Particle Output")
...
class FlipFluidDisplayEnableFluidParticlesTooltip(bpy.types.Operator):
    ...
    bl_label = _z("Enable Fluid Particles Tooltip")
    bl_description = "Enable Fluid Particles Tooltip\nEnable Fluid Particle Output"
...
class FlipFluidEnableWhitewaterSimulation(bpy.types.Operator):
    ...
    bl_label = _z("Enable Whitewater Simulation")
...
class FlipFluidDisplayEnableWhitewaterTooltip(bpy.types.Operator):
    ...
    bl_label = _z("Enable Whitewater Tooltip")
    bl_description = "Enable Whitewater Tooltip\nEnable whitewater simulation"
...
class FlipFluidEnableColorAttribute(bpy.types.Operator):
    ...
    bl_label = _z("Enable Color Attribute")
    bl_description = "Enable Color Attribute\nEnable color attribute in the Domain FLIP Fluid Surface and FLIP Fluid Particles panel"
...
class FlipFluidEnableColorMixAttribute(bpy.types.Operator):
    ...
    bl_label = _z("Enable Color Attribute + Mixing")
    bl_description = "Enable Color Attribute + Mixing\nEnable color attribute and color mixing in the Domain FLIP Fluid Surface and FLIP Fluid Particles panel"
...
class FlipFluidEnableColorAttributeTooltip(bpy.types.Operator):
    ...
    bl_label = _z("Enable Color Attribute")
    bl_description = "Enable Color Attribute\nClick to enable the color attribute in the Domain FLIP Fluid Surface and FLIP Fluid Particles panel"
...
class FlipFluidEnableViscosityAttribute(bpy.types.Operator):
    ...
    bl_label = _z("Enable Viscosity Attribute")
    bl_description = "Enable Viscosity Attribute\nEnable viscosity solver and variable viscosity attribute in the Domain FLIP Fluid World panel"
...
class FlipFluidEnableViscosityAttributeTooltip(bpy.types.Operator):
    ...
    bl_label = _z("Enable Viscosity Attribute")
    bl_description = "Enable Viscosity Attribute\nClick to enable the viscosity solver and variable viscosity attribute in the Domain FLIP Fluid World panel"
...
class FlipFluidEnableLifetimeAttribute(bpy.types.Operator):
    ...
    bl_label = _z("Enable Lifetime Attribute")
    bl_description = "Enable Lifetime Attribute\nEnable lifetime attribute in the Domain FLIP Fluid Surface and Domain FLIP Fluid Particles panel"
...
class FlipFluidEnableLifetimeAttributeTooltip(bpy.types.Operator):
    ...
    bl_label = _z("Enable Lifetime Attribute")
    bl_description = "Enable Lifetime Attribute\nClick to enable the lifetime attribute in the Domain FLIP Fluid Surface and Domain FLIP Fluid Particles panel"
...
class FlipFluidEnableSourceIDAttribute(bpy.types.Operator):
    ...
    bl_label = _z("Enable Source ID Attribute")
    bl_description = "Enable Source ID Attribute\nEnable source ID attribute in the Domain FLIP Fluid Surface and FLIP Fluid Particles panel"
...
class FlipFluidEnableSourceIDAttributeTooltip(bpy.types.Operator):
    ...
    bl_label = _z("Enable Source ID Attribute")
    bl_description = "Enable Source ID Attribute\nClick to enable the source ID attribute in the Domain FLIP Fluid Surface and FLIP Fluid Particles panel"
...
class FlipFluidHelperInitializeMotionBlur(bpy.types.Operator):
    ...
    def execute(self, context):
        if not vcu.is_blender_31():
            self.report({'INFO'}, _z("Blender 3.1 or later is required for this feature"))
            return {'CANCELLED'}

        if not context.scene.flip_fluid.is_domain_in_active_scene():
            self.report({"ERROR"}, 
                         _z("Active scene must contain domain object to use this operator. Select the scene that contains the domain object and try again."))
            return {'CANCELLED'}
        
        if context.scene.render.engine != 'CYCLES':
            context.scene.render.engine = 'CYCLES'
            self.report({'INFO'}, _z("Setting render engine to Cycles"))
        if not context.scene.render.use_motion_blur:
            context.scene.render.use_motion_blur = True
            self.report({'INFO'}, _z("Enabled Cycles motion blur rendering"))

        dprops = context.scene.flip_fluid.get_domain_properties()
        if not dprops.surface.enable_velocity_vector_attribute:
            dprops.surface.enable_velocity_vector_attribute = True
            self.report({'INFO'}, _z("Enabled generation of fluid surface velocity vector attributes in FLIP Fluid Surface panel (baking required)"))
        ...
        if not dprops.particles.enable_fluid_particle_velocity_vector_attribute:
            ...
            self.report({'INFO'}, _z("Enabled generation of fluid particle velocity vector attributes in FLIP Fluid Particles panel (baking required)"))

        if not dprops.whitewater.enable_velocity_vector_attribute:
            dprops.whitewater.enable_velocity_vector_attribute = True
            self.report({'INFO'}, _z("Enabled generation of whitewater velocity vector attributes in FLIP Fluid Whitewater (baking required)"))
        ...
        for target_object in surface_cache_objects:
            ...
            info_msg = _z("Initialized {} Geometry Node modifier on {} object").format(gn_modifier.name, target_object.name)
            self.report({'INFO'}, info_msg)
        ...
        for target_object in surface_cache_objects + fluid_particle_cache_objects + whitewater_cache_objects:
            if not target_object.cycles.use_motion_blur:
                ...
                info_msg = _z("Enabled motion blur rendering on {} object").format(target_object.name)
                self.report({'INFO'}, info_msg)

        self.report({'INFO'}, _z("Finished initializing motion blur geometry node groups and settings"))
...
class FlipFluidHelperRemoveMotionBlur(bpy.types.Operator):
    ...
    bl_label = _z("Remove Motion Blur")
    ...
    def execute(self, context):
        ...
        for bl_object in cache_objects:
            ...
            for mod in modifiers_to_remove:
                self.report({'INFO'}, _z("Removed {} Geometry Node modifier from {} object").format(mod.name, bl_object.name))
                bl_object.modifiers.remove(mod)
                is_setup_modified = True

        for bl_object in cache_objects:
            if bl_object.cycles.use_motion_blur:
                self.report({'INFO'}, _z("Disabled motion blur rendering on {} object").format(bl_object.name))
                bl_object.cycles.use_motion_blur = False
                is_setup_modified = True

        if not is_setup_modified:
            self.report({'INFO'}, _z("No motion blur setup detected"))
...
class FlipFluidHelperToggleMotionBlurRendering(bpy.types.Operator):
    ...
    bl_label = _z("Toggle Motion Blur Rendering")
    ...
    def execute(self, context):
        ...
        enable_string = _z("ON") if self.enable_motion_blur_rendering else _z("OFF")
        ...
        for bl_object in mesh_caches:
            if bl_object.cycles.use_motion_blur != enable_status:
                ...
                self.report({'INFO'}, _z("Toggled Cycles motion blur rendering on <{}> object to {}").format(bl_object.name, enable_string))
        ...
        for bl_object in mesh_caches:
            ...
            if "Input_8" in gn_modifier:
                if gn_modifier["Input_8"] != enable_status:
                    ...
                    self.report({'INFO'}, _z("Toggled motion blur on <{}> geometry node modifier to {}").format(gn_modifier.name, enable_string))

        self.report({'INFO'}, _z("Finished toggling motion blur rendering on simulation meshes ") + enable_string)
...
class FlipFluidHelperInitializeCacheObjects(bpy.types.Operator):
    ...
    bl_label = _z("Initialize Cache Objects")
    bl_description = "Initialize Cache Objects\nInitialize simulation meshes, modifiers, and data"
...
class FlipFluidHelperStableRendering279(bpy.types.Operator):
    ...
    bl_label = _z("Enable Stable Rendering")
    bl_description = ("Enable Stable Rendering\nActivate to prevent crashes and incorrect results during render."
...
class FlipFluidHelperSetLinearOverrideKeyframes(bpy.types.Operator):
    ...
    bl_label = _z("Make Keyframes Linear")
    ...
    def execute(self, context):
        ...
        try:
            ...
            self.report({'ERROR'}, _z("Override Frame value must contain keyframes for this operator to function"))
            return {'CANCELLED'}
...
class FlipFluidHelperSaveBlendFile(bpy.types.Operator):
    ...
    bl_label = _z("Save File")
    bl_description = "Save File\nOpen the Blender file window to save the current .blend file"
...
class FlipFluidHelperBatchSkipReexport(bpy.types.Operator):
    ...
    bl_label = ""
    bl_description = _z("Enable or Disable the 'Skip Re-Export' option for all objects in list")
...
class FlipFluidMakeRelativeToBlendRenderOutput(bpy.types.Operator):
    ...
    bl_label = _z("Set Relative to Blend")
    bl_description = "Set Relative to Blend\nSet a render output directory named 'render' that is located directly relative to the Blend file location"
    ...
    def execute(self, context):
        ...
        if not blend_filepath:
            self.report({"ERROR"}, _z("Cannot make path relative to unsaved Blend file"))
            return {'CANCELLED'}
...
class FlipFluidMakePrefixFilenameRenderOutput(bpy.types.Operator):
    ...
    bl_label = _z("Set Prefix as Filename")
    bl_description = "Set Prefix as Filename\nSet the render output filename prefix to match the Blend filename with a trailing underscore"
    ...
    def execute(self, context):
        blend_filepath = bpy.path.abspath("//")
        if not blend_filepath:
            self.report({"ERROR"}, _z("Cannot retrieve Blend filename of unsaved file"))
            return {'CANCELLED'}
...
class FlipFluidAutoLoadBakedFramesCMD(bpy.types.Operator):
    ...
    bl_label = _z("Auto-load Baked Frames CMD")
    bl_description = "Auto-load Baked Frames CMD\nAutomatically load frames as they finish baking when running a command line bake"
    bl_options = {'REGISTER'}
...
class FlipFluidCopySettingsFromActive(bpy.types.Operator):
    ...
    bl_label = _z("Copy Active Object Settings to Selected Objects")
    bl_description = ("Copy Active Object Settings to Selected Objects\nCopy the settings of the active FLIP object (highlighted object) to all other selected"
    ...
    def execute(self, context):
        ...
        if not src_flip_object:
            err_msg = _z("Error: No active object selected.")
            self.report({"ERROR_INVALID_INPUT"}, err_msg)
            return {'CANCELLED'}

        src_flip_type = src_flip_object.flip_fluid.object_type
        if src_flip_type == 'TYPE_NONE':
            err_msg = _z("Error: Domain type FLIP objects cannot have settings copied.")
            self.report({"ERROR_INVALID_INPUT"}, err_msg)
            return {'CANCELLED'}
        elif src_flip_type == 'TYPE_DOMAIN':
            err_msg = _z("Error: None type FLIP objects cannot have settings copied.")
            self.report({"ERROR_INVALID_INPUT"}, err_msg)
            return {'CANCELLED'}
        ...
        if not dst_flip_objects:
            err_msg = _z("Error: No other FLIP {} objects in selection to copy to.").format(flip_type_string)
            self.report({"ERROR_INVALID_INPUT"}, err_msg)
            return {'CANCELLED'}
        ...
        info_msg = _z("Copied active FLIP {} object settings to {} {} objects in selection").format(flip_type_string, str(len(dst_flip_objects)), flip_type_string)
        self.report({'INFO'}, info_msg)
        return {'FINISHED'}
...
class FlipFluidMeasureObjectSpeed(bpy.types.Operator):
    ...
    bl_label = _z("Measure Object Speed")
    ...
    def execute(self, context):
        ...
        if bl_object is None:
            err_msg = _z("No active object selected.")
            self.report({'ERROR'}, err_msg)
        ...
        if bl_object.type not in valid_object_types:
            err_msg = _z("Invalid object type <{}>. Object must be a Mesh, Curve, or Empty to measure speed.").format( bl_object.type)
            self.report({'ERROR'}, err_msg)
            print("Measure Object Speed Error: " + err_msg + "\n")
            return {'CANCELLED'}
        ...
        if len(vertices1) == 0:
            err_msg = _z("Object does not contain geometry.")
            self.report({'ERROR'}, err_msg)
            print("Measure Object Speed Error: " + err_msg + "\n")
        ...
        if len(vertices1) != len(vertices1):
            err_msg = _z("Cannot measure velocity of object with changing topology.")
            self.report({'ERROR'}, err_msg)
            print("Measure Object Speed Error: " + err_msg + "\n")
            return {'CANCELLED'}
        ...
        info_str = _z("Measure Object Speed: Finished computing <{}> vertex translations for frame {} in {}  milliseconds.").format(bl_object.name, original_frame, ms_duration, )
        print(info_str)
...
class FlipFluidClearMeasureObjectSpeed(bpy.types.Operator):
    ...
    bl_label = _z("Clear Speed Data")
    bl_description = "Clear Speed Data\nClear the measured object speed display"
...
class FlipFluidDisableAddonInBlendFile(bpy.types.Operator):
    ...
    bl_label = _z("Disable FLIP Fluids in Blend File")
    ...
    def execute(self, context):
        if bake_operators.is_bake_operator_running():
            err_msg = _z("Unable to disable addon while simulation is baking. Stop simulation bake or reload the Blend file and try again.")
            self.report({'ERROR'}, err_msg)
            return {'CANCELLED'}

        if render.is_rendering():
            err_msg = _z("Unable to disable addon while rendering. Stop the render or reload the Blend file and try again.")
            self.report({'ERROR'}, err_msg)
            return {'CANCELLED'}
        ...
        info_msg = _z("The FLIP Fluids Addon has been disabled in this Blend file. Re-enable to resume using the addon.")
        self.report({'INFO'}, info_msg)
...
class FlipFluidEnableAddonInBlendFile(bpy.types.Operator):
    ...
    bl_label = _z("Enable FLIP Fluids in Blend File")
    ...
    def execute(self, context):
        ...
        info_msg = _z("The FLIP Fluids Addon has been re-enabled in this Blend file.")
        self.report({'INFO'}, info_msg)
...
Remove an item from the list of objects for rendering"""
    ...
    bl_label = _z("Remove Item from List")
...
class FLIPFLUID_UL_passes_items(bpy.types.UIList):
    def draw_item(self, context, layout, data, item, icon, active_data, active_propname, index):
        if self.layout_type in {'DEFAULT', 'COMPACT'}:
            ...
            column1.label(text=_z(item.name), icon='MESH_CUBE')
            column2 = split.column(align=True)
            column2.prop(item, "catcher", text=_z("Catcher"), toggle=True)
...
Add a Camera Screen plane linked to the selected camera with an image or video texture"""
    ...
    def check_and_report_operator_context_errors(self, context):
        ...
        for file in self.files:
            if not file.name.lower().endswith(valid_file_types):
                ...
                errmsg = _z("Invalid file type selected: <{}>.").format(filepath)
                errmsg += _z(" Supported file types: ") + valid_types_string
        ...
        if bl_camera is None:
            self.report({'ERROR'}, _z("Camera object <{}> not found").format(hprops.render_passes_cameraselection))
            return {'CANCELLED'}
    ...
    def invoke(self, context, event):
        ...
        if 'ff_camera_screen' in context.scene.objects:
            self.report({'ERROR'}, _z("ff_camera_screen object already exists."))
            return {'CANCELLED'}
...
class FlipFluidPassesFixCompositingTextures(bpy.types.Operator):
    ...
    bl_label = _z("Fix Compositing Textures")
    bl_options = {'REGISTER', 'UNDO'}
    ...
    def execute(self, context):
        ...
        if not screen_obj:
            self.report({'ERROR'}, _z("There is no ff_camera_screen object. Please add the CameraScreen first."))
            return {'CANCELLED'}
        ...
        if "ff_camera_screen" not in [mat.name for mat in screen_obj.material_slots]:
            self.report({'ERROR'}, _z("ff_camera_screen object does not have a material named 'ff_camera_screen'."))
            return {'CANCELLED'}
        ...
        if not screen_material or not screen_material.use_nodes:
            self.report({'ERROR'}, _z("Material 'ff_camera_screen' not found or it does not use nodes."))
            return {'CANCELLED'}
        ...
        if not screen_texture_node or not screen_texture_node.image:
            self.report({'ERROR'}, _z("No valid ff_camera_screen texture node found in the material 'ff_camera_screen'."))
            return {'CANCELLED'}
        ...
        self.report({'INFO'}, _z("All compositing textures have been updated."))
        return {'FINISHED'}
