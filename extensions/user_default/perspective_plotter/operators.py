...
class MESH_OT_MatchResolutionToBG(bpy.types.Operator):
    ...
    bl_label = _z("Affects all scene cameras and their perspective guides.")
...
Reset Perspective Plotter Defaults"""
    ...
    bl_label = _z("Reset Perspective Plotter Defaults")
...
Delete Keyframes for Camera"""
    ...
    def poll(cls, context):
        ...
        fcurves = util.get_animation_fcurves(camera_obj)
    ...
    def invoke(self, context, event):
        ...
        for curve in fcurves:
            if len(curve.keyframe_points) == 0:
                fcurves_to_remove.append(curve)

        for curve in fcurves_to_remove:
            util.remove_animation_fcurve(camera_obj, curve)
...
Delete Keyframes for Camera"""
    ...
    bl_label = _z("Delete all Keyframes")
    bl_options = {'INTERNAL', 'UNDO'}
    ...
    def invoke(self, context, event):
        ...
        while(curvesToRemove):
            ...
            pointsToRemove = []
            for keyframe in keyframePoints:
                if fCurve.data_path in keyframe_search_points:
                    pointsToRemove.append(keyframe)
...
Attempt to adjust vanishing points to flatten horizon line"""
    ...
    bl_label = _z("Flatten Horizon Line")
    bl_options = {'INTERNAL', 'UNDO'}
...
class MESH_OT_SwitchtoCamera(bpy.types.Operator):
    ...
    bl_label = _z("Switch to Camera View")
    bl_options = {'INTERNAL', 'UNDO'}
