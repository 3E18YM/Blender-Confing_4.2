...
def calc_view(context, camera_obj, region):
    ...
    if camera_obj.perspective_plotter.vanishing_point_num in {'2', '3'}:
        ...
        if not (0 < horizon_intersect_percentage < 1):
            camera_obj.perspective_plotter.error_message = _z("Principal point outside vanishing points.")
            camera_obj.perspective_plotter.is_valid = False
            return
    else:
    ...
    if camera_obj.perspective_plotter.vanishing_point_num in {'2', '3'}:
        ...
        if focal_dist_2d_sqd < 0:
            camera_obj.perspective_plotter.error_message = _z("Error calculating vanishing points.")
            camera_obj.perspective_plotter.is_valid = False
            return
    ...
    if camera_obj.perspective_plotter.is_camera_sync:
        ...
        if point_intersect:
            ...
            camera_obj.perspective_plotter.error_message = _z("Could not calculate grid projection for camera.")
            camera_obj.perspective_plotter.is_valid = False
            return
...
def get_animation_fcurves(camera_obj, data_only = False):
    '''Get all animation fcurves for the current context'''
    fcurves = []
    if bpy.app.version >= (4, 4, 0):

        if camera_obj.animation_data:
            action = camera_obj.animation_data.action
            action_slot = camera_obj.animation_data.action_slot
            channelbag = anim_utils.action_get_channelbag_for_slot(action, action_slot)
            
            fcurves.extend(channelbag.fcurves)

        if camera_obj.data.animation_data:
            action = camera_obj.data.animation_data.action
            action_slot = camera_obj.data.animation_data.action_slot
            channelbag = anim_utils.action_get_channelbag_for_slot(action, action_slot)
            
            fcurves.extend(channelbag.fcurves)

    else:
        fcurves.extend(camera_obj.animation_data.action.fcurves)
        fcurves.extend(camera_obj.data.animation_data.action.fcurves)
    return fcurves

def remove_animation_fcurve(camera_obj, fcurve):
    if not fcurve:
        return
    if not camera_obj:
        return

    '''Remove an animation fcurve'''
    if bpy.app.version >= (4, 4, 0):
        if camera_obj.animation_data:
            action = camera_obj.animation_data.action
            action_slot = camera_obj.animation_data.action_slot
